...
inputs:
  ...
  - ledger/reviews/G6_REVIEW.md
  - DISPUTES.md
  - MONKEYBEE_PDF_PLAN_REVISION_7_ALIEN_AUDIT.md §§3, 10.3, 24.12–24.16, 33, 34.4–34.5, 35
status: PROPOSED
...
**Companions:** CAMPAIGN_CHARTER_REASONING_v1.1.md (proposed rationale successor), CYCLE_0_WORK_ORDER_v1.1.md (proposed work-order successor), AUDIT_FINDINGS_LEDGER_v1.1.md (proposed bounded historical-ledger successor; v1.0 remains canonical defect provenance)
...
| `R19-B01` | No repair is admitted in this draft. R20–R22 did not resolve the C6/S6 premise; the carried-forward branches recorded in `ledger/owners/G6_STATE.md` remain live pending human evidence. |
...
| `R20-A04` | C4 and C7 now carry the universal close protocol and emit sealed records #4 and #7; R21-A06 further types the seven-record sequence so C7 is not counted as candidate construction. |
...
| `R21-A01` | The existing R19-A01 repair is retained: C1 owns structural signature/ByteRange discovery, C4 owns structural impact classification, and C5 owns cryptographic trust. |
| `R21-A02` | Ledger entry #1 is now the canonical commitment-batch record/root; its manifest keeps the Charter digest and every other committed artifact independently addressable. |
| `R21-A03` | The cycle close now separates the blocking product-integrity gate from the conditional §33 evidence-admission extension, so absent stewards remove claim credit without blocking G1. |
| `R21-A04` | The existing R19-A02 repair is retained: minimum customer installability lands in C1 while the broader release program remains in C4. |
| `R21-A05` | The internally controlled build-and-submit close is now distinct from the externally controlled adjudication tail; submission never counts as an answer or verdict. |
| `R21-A06` | Records #1–#6 are construction-cycle records; #7 is a separately typed evaluation-cycle record. The foundry production-attempt distribution does not count C7 as candidate construction. |
| `R21-A07` | Charter summaries now cite the versioned `CycleTrialRecordSchemaId` owned by Work Order §3 instead of restating incomplete field subsets. |
| `R21-A08` | The existing R19-A04 repair is retained: Charter C0 points to the row-exhaustive 83-decision overlay and ratifies no default. |
| `R21-A09` | The existing R19-A07/R20-A05 repair is retained: discovery and its window close before report freeze, evaluation commitment, and measurement. |
| `R21-A10` | The day-zero held-out split remains mandatory, while a fresh independently stewarded replacement may later be sealed against a frozen candidate/protocol after adaptive exhaustion; exposed data is never relabeled held out. |
| `R22-A01` | The existing R20 scope repair is retained and made explicit: Q2 names all three admitted scopes, awards only the strongest scope actually earned, and requires the material lawfully pinnable closed/opaque systems actually measured before `observed_field_lead`. |
| `R22-A02` | G1, the C5 cycle-map result, and the C5 deliverable now bind the sellable CDR milestone to the declared R3 envelope instead of asserting unscoped product completeness; every C5 feature and gate remains. |
| `R22-B01` (grade B; premise-supported, not promoted) | The existing R20-B02 repair replaces undefined `full-field` wording with a post-report selection among the three admitted scopes and never pre-awards a scope. |
| `G6-R1-A01` | The sole surviving unscoped subsystem-completeness phrase is replaced by `the identity/evidence/report/checker stack required by the declared R0 envelope`; every C1 component, gate, exclusion, and boundary remains unchanged. |

---

## 0. What this document is, and is not

This Charter is the campaign-level staging document for MonkeyBee: it defines how many plan→beads→swarm cycles the project runs, what each cycle claims, what evidence each cycle must emit, where the internally controlled build-and-submit phase closes, and how an externally controlled adjudication tail may later answer Q2 and Q3 without self-award.
...
**Two questions this campaign exists to submit for independent adjudication; their answers are not an internally controlled campaign-end condition:**
...
- **G1 (survival wedge):** a sellable secure-ingestion product (Rev 7 §3.0: "the first commercial/product wedge is secure document ingestion and evidence-rich transformation") live at the end of Cycle 1 and improving every cycle thereafter, gated on **zero** G2/G3 evidence-admission machinery while retaining its own product-integrity gates.
...
**G1 — Survival wedge (unconditional).** R0 secure ingestion/inspection becomes sellable after Cycle 1; the declared R3 content-disarm-and-reconstruction envelope becomes sellable after Cycle 5. G1 product admission depends on zero external stewards, discovery windows, baselines, or §33 evidence credit; its clean-room, SpecCard, hostile-input, safety, installability, and drift gates remain blocking.
...
**The graceful-degradation law.** If resources, humans, or priorities collapse at any point: drop the Cycle 4 checkpoint and Cycle 7, keep everything else. The build-and-submit phase then closes as G1 — a shipped, product-evidenced, sellable product line — with no stranded work, because no G1 deliverable is gated on G2/G3 evidence admission. This law is a design constraint on every delta plan: **claim-evidence apparatus may consume cycle capacity; it may never sit on the wedge's critical path. Product-integrity gates remain mandatory.**
...
2. **Tamper-evident commitment substrate established** — independent steward custody or witnessed append-only log (Rev 7 §33.4). A bare git timestamp is insufficient. **This Charter is an independently addressable member of the first commitment batch.**
...
6. **Corpus program:** wild-tail acquisition (public corpora + crawl per Rev 7 §28), stratification, and **the main steward-sealed held-out splits created before any implementation exists**. Exposed or adaptively consumed cases become development/regression data and are never relabeled held out. After a query budget is exhausted, stronger confirmatory use requires a fresh ID and a prospectively sealed, independently stewarded replacement defined relative to a frozen candidate and evaluation protocol; that replacement does not rewrite the construction-era split.
...
| **C1** | 1–3 | L0–L2 | **R0 ships** | Kernel, immune system, autopsy wedge, read decryption, structural signature discovery, CLI, minimum installable release | Construction-cycle record #1; fuzz baseline; drift audit #1; installability evidence |
| **C2** | 3–5 | L3 + raster | — | Render engine minus real text | Construction-cycle record #2; benchmark tracking begins; D2 determinism receipts |
| **C3** | 5–7 | L4 | **R1 ships** | Text stack, ICC, divergence harness, Observatory records | Construction-cycle record #3; wild-tail gate results; divergence corpus |
| **C4** | 7–9 | L5–L6 (partial) | **R2 ships** | Writer, WASM, expanded release engineering — **Q3 checkpoint** | Construction-cycle record #4; **Q3 package submitted day 9**: committed baselines and ablations, red-team results, Evolution Trial, exact R2 boundary, ledger-to-date |
| **C5** | 9–11 | L5–L7 | **R3 ships** | Transforms, redaction, signature validation, secure output, agent surface, C ABI preview | Construction-cycle record #5; Flagship D results; declared R3 CDR envelope sellable |
| **C6** | 11–13 | L4–L7 | **R4 ships** | Fidelity track + authoring track + perf hardening + coverage scorer | Construction-cycle record #6; coverage matrix runnable; perf lanes green |
| **C7** | 13–15 | — | — | Ordered field freeze -> evaluation commitment -> measurement, when the predeclared windows have closed | Evaluation-cycle record #7; Q2 field package only after the ordered commitments; otherwise a scope-limited dated package and later addendum |

Timeline is the aggressive case; §9 states the tempo-independence rule. The build-and-submit close is internally controlled; Q3/Q2 *verdicts* belong to the later adjudication tail and land, if at all, on external calendars.
...
Gate: G2 render lanes on the non-text corpus slice; D2 receipts reproducible; the §6 product-integrity close emits construction-cycle record #2. Any §33 use additionally requires the evidence-admission extension.
...
Gate: **strict-open + render over the hostile wild-tail regression corpus, including credentialed encrypted documents** (decryption landed in C1); divergence reports generated and classified; R1 claim envelope published with page-local refusal accounting; the §6 product-integrity close emits construction-cycle record #3. When the independently stewarded sealed slice exists, its budgeted probe may satisfy the evidence-admission extension; otherwise no held-out or §33 credit is claimed.
...
Gate: the §6 product-integrity close emits construction-cycle record #4. The Q3 checkpoint additionally requires the evidence-admission extension—independent held-out probe, schema-conformant sealed record, and applicable commitment checks—before the **Q3 package is submitted to independent review on ~day 9.** Failure removes the checkpoint without blocking C4's G1 work. The verdict pends on the reviewers' calendar; the campaign continues without waiting.
...
Gate: R3 lanes green; Flagship D passes with disclosed tool lineage; secure-output D1R receipts verified; the §6 product-integrity close emits construction-cycle record #5. Any §33 use additionally requires the evidence-admission extension.
Deliverable: **the content-disarm-and-reconstruction product for the declared R3 envelope.**
...
Gate: after the three ordered stages, the §6 product/evaluation-integrity close and evidence-admission extension pass, including the independently stewarded probe and sealed evaluation-cycle record #7, before the Q2 field package is submitted. A scope-limited dated package may still be assembled around day 15; verdicts belong to the independent adjudication tail.

---

## 6. The per-cycle protocol (summary; full text in the Work Order as Rev 7 §34.9)

Every cycle C1–C7 runs the same loop, with a two-layer close:
...
5. **Swarm.**
6. **Product/evaluation-integrity close (mandatory, blocking for cycle progression or package submission):** the cycle-specific functional and security gate; drift audit (generated-truth diffs, contract-truth checks, claim-registry consistency); SpecCard coverage check; and a record under the versioned `CycleTrialRecordSchemaId` owned by Work Order §3. This layer requires no external steward and confers no §33 credit by itself.
7. **§33 evidence-admission extension (conditional, blocking only for use as Q2/Q3/§33 evidence):** an independently stewarded, budget-respecting held-out probe recorded win-or-lose; independent seal/time commitment of the cycle record; and every schema disclosure required for the claimed distribution. If unavailable, the product cycle may close, but the miss is logged, the record is excluded from claim evidence, and the affected claim downgrades rather than receiving silent credit.

No cycle's delta plan may be authored before the previous cycle's product-integrity close passes. Records #1–#6 are construction-cycle records; #7 is an evaluation-cycle record and is never pooled as candidate construction. The foundry production-attempt distribution uses only evidence-admitted construction records, while the seven-record sequence remains available for campaign lineage.
...
| Prospective, bounded, authenticated provenance | Artifact comparison support and foundry provenance; §§33.1, 33.4, and 33.18 | Ledger from day 0; Q3 `EvaluationProtocolId`; evidence-admitted cycle records under the versioned schema; commitment substrate |
...
| The claim scope | Strongest actually earned among `named_set_lead`, `registered_open_field_lead`, and `observed_field_lead`; no scope is pre-awarded, and `observed_field_lead` additionally requires the material lawfully pinnable closed/opaque systems actually measured under the same field definition |
...
**The reframe that governs the whole campaign:** each capability cycle C1–C6 produces one construction-cycle record and may contribute one foundry trial only after evidence admission; C7 produces a separately typed evaluation-cycle record. The seven-record lineage is not overhead on the way to the verdicts—it is the evidence substrate, but record #7 is never pooled as candidate construction. FrankenSim ran the same loop uninstrumented and received zero evidentiary credit for it.
...
**Endpoint identities:** `build-and-submit close` is the internally controlled end of campaign execution after the last admissible package submission or an explicit G1-only close. `adjudication tail` is the externally controlled period in which reviewers may award, reject, qualify, or never return Q2/Q3 verdicts. Submission closes execution; it never answers a question by itself.
...
This Charter becomes evidence only when committed through the tamper-evident substrate (independent steward custody or witnessed append-only log) alongside the separate discovery commitment, Q3 `EvaluationProtocolId`, historical-vintage manifest, and initial artifact/operational-boundary declaration—a git commit alone is decoration. Ledger entry #1 is the canonical commitment-batch record/root. Its manifest binds the exact Charter digest, Work Order digest, protocol commitments, and held-out seal manifests as independently addressable members; the batch root and Charter digest are never assigned the same identity. Amendments create correction-linked commitment batches that retain the prior root and amended Charter digest under §11.
