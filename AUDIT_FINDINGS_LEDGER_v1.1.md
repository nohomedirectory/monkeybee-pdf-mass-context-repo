...
**Status codes:** `OPEN-C0` (fix at Cycle 0 per Work Order §2) · `RESOLVED-CHARTER` (incorporated into Charter v1.0) · `RESOLVED-REASONING` (correction recorded in Reasoning doc) · `NOTED` (no action required) · `SOFTENED` (finding downgraded on re-examination) · `PROPOSED-CHARTER-v1.1` / `PROPOSED-KERNEL-SPLIT-v1.1` (bounded successor repair awaiting human ratification).
**Successor relation:** Carries Audit Findings Ledger v1.0 forward in full. Only the two v1.0 resolution statuses shown in the changelog are corrected, and neither becomes ratified merely by appearing here.
...
| R1-10 | Meta: the plan hand-restates its laws 4–6× each (idempotency ~25, nonce ~23), violating its own §29.5 single-source principle; R1-1 is the empirical proof of the resulting drift | A | PROPOSED-KERNEL-SPLIT-v1.1 (single home for §§24.12/24.16 with §10.3/§33; awaiting ratification) |
...
*End of proposed Ledger v1.1. Future audit rounds append through an authorized successor; v1.0 remains byte-for-byte unchanged.*
