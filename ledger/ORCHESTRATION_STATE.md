...
updated-at: 2026-07-15T00:07:00+02:00
...
- Active subgoal: `G3` gauntlet. G1 and G3 are in restarted `SELF-CHECK` after premise-first R15/R05 repairs. R24 is closed `NOT-RUN`; fresh Opus replacement R34 is independently verified `BOUND-UNSENT` against the unchanged G4 manifest and awaits the control-group commit/push before dispatch.
- Current phase: `G0/G6 DONE; G1 SELF-CHECK on R15 revision; G2/G5 SUBMIT-FOR-REVIEW with exact Sol ultra/fast replacement panes pre-staged; G3 SELF-CHECK on R05 revision; G4 SUBMIT-FOR-REVIEW / R24 NOT-RUN / R34 BOUND-UNSENT at 22749762…02c6; G7 DRAFT / recovery SELF-CHECKED with commit pending; 13 filed rounds at A=93/B=56/C=18 with 7 Sol/6 Opus; G8 INGEST with a live exact-model owner and disk checkpoint`
...
| G1 owner | Constitution v8, shell corpus, fix map | `constitution/**`, `shell/**`, `reports/FIX_APPLICATION.md`, `ledger/owners/G1_STATE.md` | REVISE | `g1b` received only committed R15 bytes plus its mechanical validation, read `AGENTS.md` first, reproduced identities, persisted receipt/transition, and keeps all 21 grades unpromoted | Premise-test all 21 items, apply only supported repairs, restart the full suite after any self-check repair, and resubmit before R16 |
...
| G4 owner | traceability matrix and C2-C7 briefs | `reports/TRACEABILITY_MATRIX.md`, `plans/cycle_briefs/**`, `ledger/owners/G4_STATE.md` | SUBMIT-FOR-REVIEW | R24 was interrupted for prohibited shell redirection before analysis or write and contributes zero coverage; fresh Opus R34 is live and untouched against unchanged pushed manifest `22749762…02c6` | Fresh G7 closes R24 `NOT-RUN`, binds R34, then root commits/pushes and dispatches replacement dependency-soundness coverage |
...
| G7 control owner | gauntlet records and convergence | `gauntlet/**`, `ledger/owners/G7_STATE.md` | DRAFT / R34 BOUND-UNSENT | G7d closed R24, allocated and bound R34 in separate persisted transitions, updated convergence without adding evidence, survived two exact-model capacity stops without fallback, corrected one hook-diagnostic absolute, and passed root's independent mechanical gate | Root commits/pushes the five-path control group, then dispatches only R34 from the sealed EOF anchor; G7 remains idle until reviewer termination |
