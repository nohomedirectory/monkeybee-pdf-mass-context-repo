...
- Inputs and identity domain: contract ID/version, lifecycle, owner layer, input domain, preconditions, budgets, authority, outcomes, evidence, determinism, falsifiers, surfaces, dependencies, exact §5 grammar-expanded prerequisite IDs and separately indexed consumers, cards, no-claims, typed `ConsequenceAxisRecord`, and operation-local `ClaimAlgebraRecord`.
- Preconditions: every presentation field validates; every required consequence axis has an exact typed value or `NotApplicable(reason_code, authority)` plus source-field link; every operation has a registered claim algebra; every dependency token expands uniquely against the closed catalog; bare-family/prose prerequisites are rejected; dependencies exist and layer direction is legal; card slots are present even when pending.
...
- Outputs and outcomes: canonical registry root, per-contract records with typed consequence axes and claim algebras, exact sorted prerequisite-edge and consumer indexes, lifecycle/migration links, and validation findings.
- Diagnostics: duplicate ID, missing field/axis/card/falsifier/surface, inferred or unjustified not-applicable axis, missing/unknown claim algebra, unknown/bare/ambiguous dependency token, range expansion outside one family, illegal dependency, unknown outcome, lifecycle contradiction, and orphan contract.
...
- Falsifiers and Gauntlet: duplicate/missing fields or axes, unjustified not-applicable value, unknown algebra/composition, dependency cycles, layer violation, prose/registry mismatch, surface widening, and migration cases.
