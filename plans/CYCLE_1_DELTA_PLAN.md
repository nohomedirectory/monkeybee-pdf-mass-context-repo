...
- Inputs and identity domain: global counters, per-item ceilings, requested reservation, resource class, hard/soft classification, explicit caller-permitted degradation modes, child-local limits, current account generation, shared-artifact producer/consumer relation, private-work reservation, and retention owner.
...
- Budgets: active C1 dimensions include source bytes, objects/versions, reference depth, container depth, total decoded bytes, per-stream decoded bytes, expansion ratio, recovery branches, canonicalization/symmetry work, diagnostics, CPU quanta, wall deadline, and output bytes. The same versioned profile also registers dormant image-pixel/plane, tile-cache, page-operator, graphics-state, form/pattern-recursion, Type-3-recursion, function-instruction, form-script event/instruction/heap/write, Static-XFA node/layout/projection, external-tool call/result-byte, font-instruction, path-segment, clip-complexity, and transparency-depth dimensions; each is unavailable in C1 and grants no execution or host capability. Child ceilings narrow the same non-resettable total account and never mint fresh capacity.
...
- Evidence and provenance: receipts bind limits profile, starting values, consumed totals, fired limit, child ownership, shared producer/logical consumers, private reservations, lookup/materialization charges, retention owner, allocation attempt/failure, refunds, and reconciliation status.
...
- Falsifiers and Gauntlet: overflow/underflow, double refund, sibling fan-out, worker replacement, recursion, shared-production double charge, free fan-out, private-reservation undercount, retention-owner duplication, cache-hit charging, allocation failure/refund, cancellation races, and conservation properties.
