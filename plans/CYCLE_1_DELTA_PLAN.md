...
- R62 owner disposition and delta evidence (2026-07-16): the chain-only allegations are owner-decided as `R62-A01=ACCEPT`, `R62-B01=NARROW`, `R62-B02=NARROW`, and `R62-B03=NARROW`. The corresponding plan delta adds RPT.009/RPT.010 to RPT.005; derives the direct-consumer index while classifying free prose as non-exhaustive navigation; records the pre-view identity-domain human gate without changing the staging DAG; and moves only the named L8-report/manifest-to-L7 check/assembly handoffs into guarded injected-data transitions with no producer import. The immutable reviewer packet and grade remain allegations; premise evidence, retained uncertainty, and byte identities are recorded in `ledger/owners/G3_STATE.md`.
...
The `Falsifiers and Gauntlet` heading is a versioned authority dependency, not a local tier ontology. Every contract record binds the exact content root and section version of `shell/MONKEYBEE_SHELL_CORPUS_v8.md` §27 required by proposed Constitution §34.9, its applicable G0–G7 and G6.1 tier rows, the applicable §27.1 falsifier-family rows, and its §27.4 release-matrix row or `NotApplicable(reason_code, authority)`. A C1 projection may narrow executed scope or mark a lane unavailable, but it may not omit or redefine the governing taxonomy; changing the taxonomy, falsifier pairing, or matrix-validity law is a declared kernel touch.
...
A `targets` segment after the prerequisite semicolon is a separately indexed assurance-coverage relation: its exact catalog tokens name contracts exercised by a target, never semantic prerequisites of that target or the exercised contract. Every FUZ.002–FUZ.010 target separately depends on FUZ.001 so rights, provenance, partition, and held-out admission precede use of any seed. Conditional alternate-mode or injected-data `consumes` clauses are separately indexed state-transition relations with an explicit source phase, target phase, and guard; they do not enter the prerequisite DAG. IMM.002 stores both relation sets, and IMM.007 proves the prerequisite DAG acyclic while also proving that every conditional transition consumes an already minted artifact and strictly advances the declared phase.

The direct-consumer index is generated only by mechanically inverting the exact expanded prerequisite-edge list; it is not ingested from free-form prose. Downstream-consumer descriptions, workflow labels, and other prose after the semicolon are non-exhaustive orientation and create no registry relation unless exact contract tokens occur inside an admitted `targets` or guarded `consumes` segment. If a described use is intended as a direct semantic input, its contract token must occur in the prerequisite segment; prose alone cannot preserve or manufacture that edge.

Within the assurance path admitted by controlling Rev 7 §8.1 and preserved in proposed Constitution §8.1, an already minted immutable L8 report or manifest artifact may be presented to the lower-owned C1 check or private-package assembly named by an explicit guarded `consumes` relation only when that relation classifies the artifact as injected data and forbids importing or invoking the producer implementation or surface. Such injection stays outside the prerequisite DAG and runtime dependency graph, advances only after the artifact is minted, and admits no other higher-to-lower relation. IMM.007 validates the artifact-before-transition receipt, phase/guard, data-only qualifier, and absence of a producer import. This grammar is about semantic contract dependencies and coverage only; it does not create implementation tasks.
...
- `C1.CC.FDN.017@1` — host-output publication capability and receipt.
- `C1.CC.FDN.018@1` — dormant embedded-program meta-contract.
...
- `C1.CC.IMM.021@1` — pre-run evaluation-protocol commitment interface.
- `C1.CC.IMM.022@1` — human-owned vulnerability-response policy status.
...
Three pre-view operations remain activation-blocked pending an explicit higher-law identity-domain disposition. The proposed existing-domain classification treats REV.008 as revision discovery over `SourceRootId`, `BaseRevisionGraphId`, and its declared frontier manifest; BYT.005 as a source/base-graph/exact-fact overlay; and SEC.008 as planning over `SourceRootId`, `BaseRevisionGraphId`, `DocumentViewBasisId`, and `SecurityContextId`. Each output is later bound into the finalized graph and exact `DocumentViewId`; none may claim selected-view scope early. Because controlling Rev 7 §9.6 also literally names signature, preservation, and security operations as view-bound, this plan does not self-ratify the classification: §11.2 requires a human record admitting these staging operations under the revision-discovery/declared-domain law, or a versioned kernel change if that reading is rejected. Adding the later `DocumentViewId` as a staging prerequisite is forbidden because it would create a dependency cycle rather than reconcile authority.
...
- Determinism and replay: D1 Serialized for the same typed input and protocol; insertion, allocation, traversal, locale, and thread order cannot affect canonical bytes. The decoded typed value and its semantic root remain D0 Structural.
...
- Inputs and identity domain: global counters, per-item ceilings, one operation-wide project-accounted concurrent-live-memory ceiling, requested reservation, resource class, hard/soft classification, explicit caller-permitted degradation modes, child-local limits, current account generation, shared-artifact producer/consumer relation, private-work reservation, and retention owner.
...
- Budgets: active C1 dimensions include project-accounted concurrent live memory bytes, source bytes, objects/versions, reference depth, container depth, total decoded bytes, per-stream decoded bytes, expansion ratio, recovery branches, canonicalization/symmetry work, diagnostics, CPU quanta, wall deadline, and output bytes. The live-memory dimension is one operation-wide ceiling over project-controlled simultaneously live allocation capacity: reservations precede allocation, child leases borrow or narrow the same ceiling, charges remain until linear release or retention-owner transfer, and receipts preserve a monotone high-water value. It is not a claim about physical resident/committed memory or dependency-internal allocation. The same versioned profile also registers dormant image-pixel/plane, tile-cache, page-operator, graphics-state, form/pattern-recursion, Type-3-recursion, function-instruction, form-script event/instruction/heap/write, Static-XFA node/layout/projection, external-tool call/result-byte, font-instruction, path-segment, clip-complexity, and transparency-depth dimensions; each is unavailable in C1 and grants no execution or host capability. Child ceilings narrow the same non-resettable total account and never mint fresh capacity.
...
- Outputs and outcomes: reserve, consume, split, refund, and close return typed receipts. A shared immutable artifact charges physical production once to its producing account, records every logical consumer without multiplying that counter, charges bounded lookup/materialization/output work on hits, reserves worst-case concurrent private work before launch, and assigns exactly one retention charge owner. `FallibleAllocationLease` charges the operation-wide current live-memory reservation before one bounded project-controlled allocation attempt; typed allocation failure refunds or reconciles the lease, while success retains that charge until linear release or retention-owner transfer, and either path returns the owning contract's refusal/limit or allocation receipt. A hard limit stops before its invariant would be violated, while a soft limit may return only an explicitly caller-permitted partial or degraded outcome with exact covered/missing scope and fired limit. No degraded result is `Complete`.
...
- Evidence and provenance: receipts bind limits profile, starting values, consumed totals, fired limit, child ownership, shared producer/logical consumers, private reservations, lookup/materialization charges, retention owner, live-memory ceiling/current/high-water values, allocation attempt/failure, linear release or retention transfer, refunds, and reconciliation status.
...
- Falsifiers and Gauntlet: overflow/underflow, double refund, sibling fan-out, worker replacement, recursion, heterogeneous simultaneously live allocations, child live-memory multiplication, early/double release, high-water undercount, retention-transfer loss/double charge, shared-production double charge, free fan-out, private-reservation undercount, retention-owner duplication, cache-hit charging, allocation failure/refund, cancellation races, and conservation properties.
...
- Inputs and identity domain: requested claim scope, candidate values or alternatives, coverage partitions, the closed facet registry (`ByteExact`, `ObservedBytes`, `DeterministicallyDerived`, `BoundedApproximation`, `UnboundedApproximation`, `NormativeBasis`, `ProfileDefined`, `CompatibilityObserved`, `Recovered`, `Heuristic`, `ExternalCheck`, `CallerAttestation`, `EmpiricalMeasurement`, `MechanizedProof`, `CryptographicCheck`), keyed `EvidenceSet`, provenance, artifact availability, optional `OracleLineageId` for every corroborating observation, typed `ClaimAlgebraRecord`, diagnostics, and issuer capability.
- Preconditions: the issuer may mint only its admitted facet class and scope; every facet payload validates against its registry schema and lifecycle. `EvidenceSet` keys bind named subclaim, metric, region, profile, source snapshot, and protocol version; coverage is disjoint and exhaustive relative to the requested scope. Every operation registers required inputs, scope propagation, approximation metric, and refusal conditions; a missing or unknown composition fails closed. Compatibility-observation and empirical-measurement issuance remain unavailable absent their own precommitted rights-admitted protocols.
...
- Outputs and outcomes: canonical algebra is singular scoped success, partial coverage, retained alternatives, or value-less refusal; terminal cancellation/indeterminacy stay separate. The validated `EvidenceSet` is canonically key-ordered, deduplicates only byte-identical observations with the same lineage-bearing payload, retains distinct corroboration, and refuses incompatible same-key payloads or raises an explicit higher-level ambiguity. Availability is one of `Embedded` (checker bytes enclosed), `ExternallyRetrievable` (named immutable authority/capability/policy/expected identity), `Recomputable` (admitted inputs, versions, and bounded procedure), `IdentityOnly` (identity authenticable but bytes not materializable), or `Unavailable` (known missing). `OracleLineageId` records known correlation and unknowns without assigning a numeric independence score.
- Diagnostics: identify invalid key/coverage, incompatible evidence, unlicensed or dormant facet issuance, unknown composition, invalid availability transition, unavailable decision-critical material, and missing recomputation/materialization receipt.
- Evidence and provenance: each facet retains typed payload, issuer capability, lifecycle, origin, protocol, lineage, and withdrawal state. Weakness, recovery assumptions, heuristic status, and availability propagate through the registered `ClaimAlgebraRecord`; retention, eviction, retrieval, recomputation, materialization, or refusal emits a receipt, and an identity digest never implies byte availability.
...
- Falsifiers and Gauntlet: omitted registry facet, malformed payload, contradictory keyed coverage, false deduplication, lost distinct corroboration, incompatible bounds, fabricated/dormant issuance, unknown algebra, availability upgrade, identity-as-content, alternative collapse, partial-as-singular serialization, missing/forged lineage, correlated-wrapper multiplication, and evidence-laundering properties.
...
- Outputs and outcomes: returns rooted origin/location IDs, conversion receipts, or typed refusal for cross-root ambiguity, cycle, unavailable parent, or budget exhaustion. For object/version findings it also emits `ObjectExplanationHandoffId`, a typed subgraph over exact source/graph/view/object, derivation, virtual/enclosing-span, and visible/shadowed/unreachable/unknown occurrence nodes; unresolved inverse causality remains a first-class gap and no dedicated explanation query is created in C1.
...
- Evidence and provenance: every conversion binds input/output IDs, algorithm/profile, limits, card slots, evidence facets, and no-claim boundary; the handoff binds the exact node/edge schema, retained gaps, and owning later-cycle explanation contract.
...
- Falsifiers and Gauntlet: cross-root aliasing, virtual-as-file span confusion, cyclic origins, missing parents, symmetric duplicates, lost occurrence state, enclosing-span promoted to unique cause, handoff graph drift, and serialization-order tests.
- Surfaces: tokens, COS values, xref/revision records, decoded streams, diagnostics, reports, C1 object-provenance projections, later explanation handoff, checker, and fuzz witnesses.
...
- Inputs and identity domain: proper-domain parent identities, algorithm/profile versions, semantic limits, actual dependency/selection manifest, build/target identity where required, run observations, and the qualified determinism registry: `D0 Structural`, `D1 Serialized`, `D1R EntropyExternalSerialized`, `D2 SameTargetPixels`, `D3 CrossTargetBounded`, `D4 Statistical`, and `D5 Fast`.
...
- Outputs and outcomes: returns one qualified determinism class, admitted equivalence relation, capture/availability record, and partial/refused replay reasons. D0 covers structural/report identity; D1 covers byte-identical canonical serialization under pinned inputs; D1R records reproducible semantics without byte identity when entropy or mutable external input participates; D2 covers same-target pixel identity; D3 a declared cross-target envelope; D4 the full seeded attempt distribution; and D5 permitted recorded variation. These names are disjoint from campaign decisions D1/D2.
...
- Determinism and replay: C1 semantic structures target D0, canonical serialized bytes use D1 where all inputs are pinned, and byte decoding is deterministic under admitted inputs; performance timing is never part of D0 or D1.
- Falsifiers and Gauntlet: unknown or unqualified class, D0/D1 collapse, variable input mislabeled D1, cross-thread/order tests, manifest-change tests, timing-envelope changes, missing-secret cases, capture truncation, and repeatable-wrong-result separation.
...
- Inputs and identity domain: candidate index key, proper-domain parent identity, actual dependency/selection manifest, tenant/security/sensitivity domain, collection key source, hasher/ordering policy, producing `ResourceAccount`, logical consumer attribution, lookup/materialization lease, and retention owner.
...
- Outputs and outcomes: hit requires full manifest revalidation and a charged lookup/materialization receipt; otherwise miss, refused, stale, quarantined, or private-failure state is returned. One immutable entry has one physical producer charge and one retention owner; consumer attribution never multiplies that charge or grants free output work.
...
- Evidence and provenance: entry receipt binds producer operation/account, proper-domain inputs, actual dependencies, security context identity, sensitivity, size, logical consumers, lookup/materialization/output work, private reservations, retention owner, and eviction/reconciliation state.
...
- Falsifiers and Gauntlet: hash-flood inputs, priority/absence changes, producer double charge, free consumer fan-out, missing private reservation, duplicate retention owner, cross-tenant timing attempts, credential crossing, source change, cancellation, and concurrent insert/evict races.
...
- Inputs and identity domain: schema ID/version, field definitions, criticality, defaults, extension rules, canonical encoding, prior versions, migration policy, and `StabilityActivationRecord` for each surface that claims compatibility.
...
- Outputs and outcomes: returns migrated artifact plus loss/coverage receipt, unchanged compatible view, or refusal for ambiguous/unsupported/lossy change. `StabilityActivationRecord` is `NoStabilityPromise`, `StableWithinMajor(scope, prior_surface_root, activation_authority)`, or `Withdrawn`, and activates compatibility comparison only for its named schema, diagnostic, CLI, or later public-API surface.
...
- Falsifiers and Gauntlet: unknown fields, field repurposing, downgrade/upgrade loops, duplicate semantic fields, protected-field loss, claimed stability without activation/prior root, incompatible change accepted within an active scope, and producer/checker drift.
...
- No-claim boundary: a successful structural migration does not prove that old evidence remains current or that newly added fields were historically observed; C1 makes no general Rust-facade or C-ABI stability promise.
...
- Identity, lifecycle, layer: active C1 L0/L8 host-boundary contract; one `HostOutputCapabilityId` names the destination authority, publication class, crash envelope, and protocol version.
- Inputs and identity domain: admitted destination capability, sink kind, ownership/permission/overwrite policy, `Buffered`/`AtomicVisibility`/`DurableCommit` request, destination-filesystem relation where applicable, disclosure policy, limits, and fault observations.
- Preconditions: the host authenticates the narrow destination capability and exact supported commit operations; an unsupported atomicity or durability premise yields a weaker admitted class or refusal before publication language is issued.
- Card slots: `SC.C1.FDN.HOST-OUTPUT-PUBLICATION.001` — `PENDING-LICENSED-SOURCE`; this project-law slot contains no external standard semantics.
- Budgets: output bytes, private-buffer or temporary-file bytes, path components, short-write retries, flush/durability work, cleanup, diagnostics, and receipt bytes are bounded.
- Cancellation: before visibility, abort/cleanup retains private or unknown remnants explicitly; after host acknowledgement, cancellation cannot invent rollback and returns committed or indeterminate state.
- Authority and secrets: the capability is destination-scoped, non-copyable, no-follow/no-clobber where supported, and cannot grant PDF writing, ambient filesystem traversal, declassification, or publication beyond the caller-approved sink.
- Outputs and outcomes: `HostPublicationReceiptId` binds requested/admitted class, private/visible state, data and directory durability observations, destination identity, commit/abort result, cleanup state, and weaker/refused/indeterminate outcome.
- Diagnostics: short write, quota or space exhaustion, flush failure, directory-durability gap, symlink/path replacement, pre-existing-destination race, permission/ownership change, cross-filesystem rename, unsupported directory primitive, broken pipe, and post-visibility failure.
- Evidence and provenance: receipt binds capability issuer, destination/sink class, protocol, exact bytes or semantic root offered, host guarantees and observations, fault/cleanup trace, limits, and disclosure state.
- Determinism and replay: semantic output identity follows its owning contract; host publication observations are run evidence and never enter D0, while D1 applies only to the serialized bytes themselves under pinned inputs.
- Falsifiers and Gauntlet: every diagnostic class above, cancellation at each phase, crash before/after acknowledgement, no-clobber concurrency, weaker-sink admission, false rollback, and forged destination capability.
- Surfaces: RPT.005 projection, IMM.016 package assembly, CLI.006 output, saved reports, and any later host publication adapter; no PDF writer surface is created.
- Dependencies and consumers: depends on FDN.005–FDN.010/FDN.012/FDN.015; RPT.005, IMM.016, CLI.006, and FUZ.010 consume it. Calling surfaces carry the resulting receipt into their already-required FDN.006 terminal accounting; FDN.006 does not depend on or directly consume FDN.017.
- Migration and withdrawal: host protocol, path law, publication class, or crash-envelope change versions the capability/receipt; stronger language requires fresh host evidence.
- No-claim boundary: a receipt proves only the declared sink operation and observed envelope, not source/report truth, PDF durability inside another system, or rollback after visibility.
- Rationale: publication classes require an evidence-producing host contract rather than labels supplied by callers.

#### `C1.CC.FDN.018@1` — Dormant embedded-program meta-contract

- Identity, lifecycle, layer: dormant kernel contract in L0; its registry and laws are frozen in C1 while every executable tenant remains unavailable.
- Inputs and identity domain: versioned executable-feature registry, tenant/language identity, narrowed capability manifest, deterministic host-fact snapshot, operand/call/event limits, dependency manifest, cancellation policy, effect-journal schema, sanitizer visibility, and termination law.
- Preconditions: C1 admits no tenant. A later owner must supply reviewed semantics, active cards, exact tenant capability profile, registry entry, interpreter contract, sanitizer coverage, and kernel-compatible gate evidence before activation.
- Card slots: `SC.C1.FDN.EMBEDDED-PROGRAM-META.001` — `PENDING-LICENSED-SOURCE`; no language or tenant semantics are supplied.
- Budgets: operand/call stacks, instructions, output, allocations, effects, recursion/cycles, event fan-out, trace, and journal bytes are registered dimensions under FDN.005 but have zero executable C1 authority.
- Cancellation: a future tenant must stop at bounded checkpoints, reconcile accounts, and atomically commit or discard its effect journal; C1 exposes no execution path.
- Authority and secrets: no ambient host I/O, network, filesystem, process, UI, clock, locale, randomness, dynamic code, or external-resource discovery; only explicit pure intrinsics and tenant-specific narrowed leases may ever be admitted.
- Outputs and outcomes: dormant registry record and later-facing contract seam for deterministic result, typed refusal/cancellation/indeterminacy, termination trace, and atomic effect-journal state; C1 outcome is `Unavailable` for every tenant.
- Diagnostics: unregistered tenant/feature, capability widening, nondeterministic host fact, missing limit, event storm, hidden effect, sanitizer mismatch, dynamic-code request, and attempted C1 execution.
- Evidence and provenance: registry records tenant owner, semantic/card authority, capability profile, dependency manifest, limits, sanitizer coverage, version, and activation/withdrawal evidence; all tenant rows are pending.
- Determinism and replay: future class follows FDN.012 under exact host facts/dependencies/limits; C1 has no run or replay claim.
- Falsifiers and Gauntlet: ambient-authority attempt, lease widening, unmetered stack/instruction/effect, hidden executable construct, sanitizer omission, partial journal publication, cancellation race, and any reachable C1 tenant.
- Surfaces: kernel registry and future interpreter/sanitizer contracts only; no C1 CLI, report behavior, execution, rendering, font, form, or host-action surface.
- Dependencies and consumers: depends on FDN.004–FDN.012/FDN.015/FDN.016; later executable tenants and sanitizers consume it, while IMM.002/IMM.009 verify that no active C1 surface claims a tenant.
- Migration and withdrawal: meta-contract change is a kernel version bump with migration and claim-lapse review; tenant changes version only that tenant when the shared law is unchanged.
- No-claim boundary: a dormant registry seam supplies no interpreter, semantic coverage, sanitization result, or safety claim.
- Rationale: reserving the shared least-authority law now prevents later executable domains from inventing incompatible budget, authority, effect, and sanitizer rules.
...
- Inputs and identity domain: exact `SourceRootId` and `BaseRevisionGraphId`, BYT.006 pre-graph facts, REV.008 structural signature ranges, parse ownership, explained/unexplained state, revision status, sensitivity, raw-preservation need, and carry/drop intent.
- Preconditions: the §11.2 pre-view identity-domain reconciliation has been human-ratified for this exact existing domain; the base graph and every input fact already exist; every facet issuer is authorized; overlapping facets are allowed and never collapsed into one exclusive label.
...
- Dependencies and consumers: depends on REV.004/REV.005, DOC.010, SYN.004–SYN.007, FLT.001–FLT.010, SEC.001/SEC.008, and conditionally SEC.004–SEC.009 for declared encrypted contexts; REV.008/REV.009, DOC.001, and later recovery/report contracts consume it.
...
- Dependencies and consumers: depends on FLT.001–FLT.009, FDN.001–FDN.014, SEC.001/SEC.008, and conditionally SEC.009 for decrypted material; REV.006/DOC.001 consume it directly, while RPT.001–RPT.003/RPT.007/RPT.009 are indirect downstream projections through their declared document/security/report prerequisites. REV.002 uses only FLT.002 bootstrap-stage provenance and is not a decoded-container consumer.
...
- Inputs and identity domain: exact `SourceRootId`, `BaseRevisionGraphId`, `DocumentViewBasisId`, and SEC.001 `SecurityContextId`; optional admitted handler context; duplicate-preserved raw `/StmF`/`/StrF`/`/EFF`/`/EncryptMetadata` and per-stream Crypt/Identity declarations; metadata policy; object category; xref/object-stream rules; and cards.
- Preconditions: the §11.2 pre-view identity-domain reconciliation has been human-ratified for this exact existing domain; raw dictionary occurrences, `BaseRevisionGraphId`, and `DocumentViewBasisId` are exact; final `DocumentViewId` is not an input; all scope rules are later card-reviewed. An unencrypted/Identity context requires no SEC.002–SEC.004 credential/key result.
...
- Dependencies and consumers: depends on applicable BYT.001–BYT.006, SYN.001–SYN.007, REV.001–REV.009, FLT.001–FLT.010, SEC.001–SEC.009, and DOC.001–DOC.008/DOC.010–DOC.015 contracts; RPT.001/RPT.003/RPT.007/RPT.010 and REC.002 consume its strict result.
...
- Inputs and identity domain: exact immutable DOC.009 `StrictOpenResultId` with its source/final-strict-graph/view alternatives and observed coverage, recovery request, exact FDN.007 `RecoveryPolicy` view/profile version, caller authority, and limits.
...
- Evidence and provenance: admission receipt binds the strict result, source/final strict graph, strict view alternatives, recovery profile, caller/task, capabilities, observed coverage, and search budget.
...
- Dependencies and consumers: depends on DOC.009 and FDN.004–FDN.012; REC.001/REC.003–REC.010 consume it, while RPT.001 may later compose the resulting recovery roots without becoming an admission prerequisite.
- Migration and withdrawal: recovery-profile change versions admission; old strict results never acquire recovery outcomes in place.
...
- Identity, lifecycle, layer: active C1 L2/L8 `OpenReport` family contract; every semantic body is an immutable derived artifact with one exact schema/version/root and an explicit supersession relation when a later recovery-aware body is minted.
- Inputs and identity domain: exact `SourceRootId`, source identity/availability, every produced base/final strict revision graph plus discovery protocol/version and supersession relation, strict `DocumentViewId` or strict ambiguity, DOC.009 strict result and `OccurrenceVisibilityMapId`, DOC.005 page-attribute/geometry/continuity inventory, immutable RPT.003 strict security inventory, RPT.010 C1 validation report, DOC.012 encrypted-wrapper inventory, DOC.013 UTF-8-string inventory, DOC.014 `NeedsRendering` inventory, DOC.015 metadata inventory, exact registered recovery status, limits, diagnostics, and active operation capabilities; when recovery is requested or admitted, also the exact RPT.002 `RecoveryReportId`, RPT.007 `RecoverySecurityEnrichmentId`, and every represented revision/view/hypothesis plus typed analyzed-or-unavailable reachability/effect coverage.
- Preconditions: all referenced strict artifacts exist and are root/graph/view consistent and remain immutable; missing decision-critical strict or recovery artifacts carry explicit availability. A recovery-aware body represents every live alternative or its exact unavailable coverage, and it never rewrites an earlier body or any strict input.
...
- Outputs and outcomes: immutable `OpenReportId` records source class/ranges; every produced base/final strict graph identity, discovery protocol/version, observed-range coverage, limits/assumptions, and supersession relation; strict selection or ambiguity; strict status; exact registered recovery status; every visible/shadowed/unreachable/unknown occurrence classification and frontier; page structural/geometry/continuity evidence; encryption; structural signatures; the bounded DOC.012–DOC.015 inventories and RPT.010 validation layers or their explicit unavailability; unexplained bytes, resource use, and no-claims. When recovery is requested or admitted, the body also binds RPT.002/RPT.007 and records every admitted revision/view/hypothesis with per-node trigger, target, revision, visibility, required host capability, executable-feature class, sanitization state, evidence/provenance, and explicit analyzed-or-unavailable coverage; when recovery is not requested, that status and the inapplicable recovery projection are explicit rather than inferred.
...
- Determinism and replay: the semantic body targets D0 under the exact strict artifacts, recovery status, represented RPT.002/RPT.007 roots, protocol/cards, and semantic limits; a later recovery projection mints a new body/root, while RPT.004 run observations remain outside semantic identity.
- Falsifiers and Gauntlet: mandated-field omission, omitted base/final graph or supersession relation, graph/view mismatch, strict-artifact mutation, missing or ambiguous recovery status, omitted live hypothesis, analyzed/unavailable coverage confusion, candidate-specific security generalized across alternatives, partial-as-all-scope, diagnostic truncation, secret leak, unknown fields, and producer/checker canonical-root tests.
- Surfaces: Rust report API, CLI inspect JSON/JSONL/human rendering, evidence package, capability manifest, and close gate.
- Dependencies and consumers: depends on BYT.001–BYT.006, REV.001–REV.009, DOC.001–DOC.015, SEC.001–SEC.009, RPT.002/RPT.003/RPT.007/RPT.010, and FDN.001–FDN.015 outputs for the represented strict or recovery-aware composition; RPT.004/RPT.005/RPT.009 consume the runtime report, IMM.004 consumes its contract declaration through IMM.002, and IMM.016 accepts the immutable report root only through its guarded injected-data relation.
- Migration and withdrawal: schema change follows FDN.015; a later recovery-aware body mints a new report root and explicit supersession link, while every older root remains immutable and its claim rows may lapse without deletion.
- No-claim boundary: the report covers only executed strict and admitted recovery contracts, represented alternatives, observed ranges, credentials, cards, and budgets; recovery status does not establish exhaustive search, singular source truth, whole-document security, or absence across unavailable coverage, and no composition mutates an underlying strict or recovery artifact.
- Rationale: composing the candidate OpenReport grammar over separately immutable strict and recovery roots preserves strict evidence while binding mandatory status, alternatives, and typed coverage into the report-family identity.
...
- Dependencies and consumers: depends on REC.001–REC.010 and FDN.001–FDN.015; RPT.004/RPT.005/RPT.007 consume the runtime report, IMM.004 consumes its contract declaration through IMM.002, and IMM.016 accepts the immutable report root only through its guarded injected-data relation.
...
- Inputs and identity domain: validated strict/recovery/security/validation/replay/enrichment report or registry object, target projection/version, disclosure policy, exact FDN.017 `HostOutputCapabilityId`, requested publication class, and limits.
- Preconditions: source object passed its schema constructor; projection is generated from the normative registry; unknown-critical fields follow schema law; the requested publication class is admitted by the supplied host-output capability or explicitly narrowed/refused.
...
- Outputs and outcomes: versioned JSON, JSONL event stream, or human view plus schema/root metadata, exact FDN.017 `HostPublicationReceiptId`, admitted class, visibility/durability state, and truncation/terminal state; a post-visibility failure is committed or indeterminate rather than fictional rollback.
...
- Evidence and provenance: projection receipt binds source semantic ID, projection version, disclosure policy, FDN.017 capability/publication roots, output identity, visibility/durability observation, and losses/redactions.
- Determinism and replay: canonical JSON bytes are D1 Serialized where declared; their decoded semantic body is D0 Structural. Human formatting is deterministic for a pinned renderer but not an identity substitute.
...
- Dependencies and consumers: depends on FDN.002/FDN.008/FDN.015/FDN.017 and RPT.001–RPT.004/RPT.007/RPT.009/RPT.010; CLI and RPT.008 consume projection artifacts, IMM.004 consumes this contract's declaration through IMM.002, and IMM.016 accepts applicable projection artifacts only through its guarded injected-data relation.
...
- Inputs and identity domain: active contract registry, card coverage, build/toolchain/dependency identities, enabled profiles/features, surface projections, structured source/dialect/filter/decryption/report/CLI/font/color-ICC/validation-profile/determinism/host-adapter categories, declared assurance/corpus requirements and availability, and no-claim/lifecycle state.
...
- Outputs and outcomes: each row reports source types, dialect/read profiles, filters, decryption profiles with core-versus-unverified/pending technical-specification classification, reports, CLI surfaces, font types, color/ICC coverage, validation profiles, determinism classes, host adapters, known refusals/no-claims, and lifecycle state (`Available`, `Absent`, `Disabled`, `DecisionBlocked`, `Unsupported`, or `Unavailable`). Assurance and corpus rows state only declared/required/unavailable scope and partitions; observed pass/fail and executed coverage belong only to RPT.008.
...
- Falsifiers and Gauntlet: missing category/codec/algorithm, core/technical-specification collapse, candidate row claiming observed assurance or executed corpus coverage, absent/blocked category advertised available, disabled feature still advertised, friendly-name alias, surface widening, withdrawn contract, and generator drift.
...
- Identity, lifecycle, layer: active C1 L2/L8 recovery-projection contract; it links immutable DOC.009/RPT.003 strict artifacts to recovery-only findings without modifying those roots and without depending on an `OpenReportId`.
- Inputs and identity domain: exact DOC.009 strict result with source/final-strict-graph/view alternatives, RPT.002 recovery report, RPT.003 strict security inventory, REC.009 analysis record, all live candidate-view consequence receipts/selections, DOC.012–DOC.015 candidate inventories or explicit unavailability, cross-hypothesis security/materiality results, schema, and limits.
- Preconditions: all inputs share one source and strict parent graph; every strict artifact root is retained unchanged; every live recovery alternative is represented or explicitly unavailable.
...
- Outputs and outcomes: immutable `RecoverySecurityEnrichmentId` linking the strict result/graph/view roots, recovery report, strict inventory, candidate-specific findings, across-alternative conclusions, affected scope, explicit ambiguity, and no-claims; it supplies the exact recovery-status and analyzed-or-unavailable reachability/effect projection consumed by RPT.001. An unavailable link stays unknown and is never evidence that no alternatives exist.
...
- Falsifiers and Gauntlet: strict-artifact mutation, OpenReport input smuggling, alternative omission, candidate-specific security generalized to all, stale selection, partial-as-full coverage, unavailable-as-absence laundering, and checker disagreement.
- Surfaces: OpenReport recovery projection, recovery-aware inspect output, CLI recovery, evidence package, held-out probe, trial record, and close gate.
- Dependencies and consumers: depends on RPT.002/RPT.003, REC.001–REC.010, DOC.008/DOC.009, DOC.012–DOC.015, and FDN.001–FDN.015; RPT.001/RPT.004/RPT.005/RPT.009, IMM.004/IMM.016, and the close gate consume it.
...
- Rationale: an OpenReport-independent recovery projection preserves strict-first evidence, avoids a contract dependency cycle, and supplies the mandatory recovery-specific security/behavior facts to the OpenReport family.
...
- Inputs and identity domain: exact RPT.006 candidate manifest root, exact IMM.012 `DriftAuditResultId`, exact FUZ.011 reachable-panic result, FUZ.015 `AssuranceLaneAggregateId`, `CheckerAdmissionState` (`Assessed(IMM.020 CheckerLineageAssessmentId)` or authenticated `Unavailable(reason, affected_rows, evidence)`), row-scoped human gate states including IMM.022 when customer availability is requested, checked contract/card/claim/schema/surface/build roots, audit/assurance policy versions, lifecycle state, and availability.
- Preconditions: candidate manifest was finalized before audit or assurance; every supplied later receipt binds that unchanged candidate/build and its mandatory row scope; no unresolved blocking, missing, cancelled, stale, failed, or indeterminate result is hidden or relabeled. `Unavailable` is admissible only for explicitly checker/package/independence-dependent rows and blocks those rows; it cannot satisfy their evidence requirement or block unrelated R0 rows by itself.
...
- Outputs and outcomes: immutable `CapabilityAdmissionEnvelopeId` with manifest root, drift/panic/lane roots, checker state, applicable human gate-status roots, exact per-row admitted/blocked/indeterminate status, observed Gauntlet/corpus evidence only where supplied, decision blocks, missing-lane state, availability, and an overall product projection that may admit unrelated rows while withholding checker-dependent or customer-facing rows.
- Diagnostics: root mismatch, audit-before-manifest, missing truth check, unresolved blocking finding, stale build/registry, lifecycle contradiction, unauthenticated or overbroad checker unavailability, checker-dependent row admitted without assessment, unavailable evidence, and projection failure.
...
- Falsifiers and Gauntlet: self-audited manifest root, audit/assurance-result injection into candidate manifest, stale-root substitution, missing/cancelled/indeterminate lane treated as pass, checker absence globally blocking unrelated rows, checker-dependent row admitted without IMM.020, omitted panic or checker miss, blocked-row advertisement, lifecycle drift, and cancellation.
...
- Dependencies and consumers: depends on RPT.005/RPT.006, IMM.012, FUZ.011/FUZ.015, and FDN.015; in state transition `candidate-audit-available -> checker-assessed-admission-available`, additionally consumes IMM.020 only when the requested row claims assessed checker lineage and supplies the matching root; in state transition `internal-product-admission-available -> customer-availability-eligible`, additionally consumes IMM.022 only when customer availability is requested and the matching adopted status is supplied. CLI.008, IMM.016/IMM.019, and the product/close gates consume the row-scoped envelope.
...
- Dependencies and consumers: depends on FDN.008/FDN.009/FDN.012/FDN.015, RPT.001–RPT.004/RPT.007/RPT.010, and the exact triggering artifact contract; RPT.004/RPT.005 projections, FUZ.009, IMM.004/IMM.016, CLI.005, and the close gate consume its schema or identity.
...
- Inputs and identity domain: exact `DocumentViewId`, source-backed `RevisionGraphId`, role-labeled validation `DerivationId`, exact checker build identity, checker protocol identity/version, checker schema identity/version, active profile and profile version, active rule-set identity/version, DOC.007 per-rule implementation identities/versions and results for structural consistency, declared-constraint consistency, and independently sourced model checks, DOC.009 strict coverage, rule locations, and artifact availability.
- Preconditions: the view/graph/validation-derivation/checker-build/checker-protocol/checker-schema/profile/rule-set identities are present, immutable, and mutually bound; every normative rule is backed by an active reviewed card and exact implementation identity/version; each layer executes and reports separately; an unrun or unsupported rule cannot pass.
...
- Outputs and outcomes: immutable `ValidationReportId` with exact view/graph/role-labeled-validation-derivation/checker-build/checker-protocol/checker-schema/profile/profile-version/rule-set identities; separate layer 1–3 summaries; per-rule implementation identity/version; canonical per-rule `Pass`, `Fail`, `NotApplicable`, `NotEvaluated`, `Unsupported`, or `Indeterminate`; separate severity and coverage; typed source, virtual-span, object, semantic, or global locations; explicit C1 external-comparison status `unavailable/not run`; unsupported rule classes; limits; diagnostics; and availability.
- Diagnostics: missing or mismatched validation derivation, checker build, checker protocol/schema, view/graph/profile/rule-set, or per-rule implementation identity; inactive/pending card, unknown rule, duplicate outcome, layer collapse, illegal outcome transition, missing severity/coverage/location, unsupported class, cancellation, disclosure refusal, and canonicalization failure.
- Evidence and provenance: every result binds view/graph/validation-derivation/checker-build/checker-protocol/checker-schema/profile/rule-set/layer/rule/card and per-rule implementation identities, per-rule algebra, input evidence and typed locations, exact observed coverage, availability, and construction receipt.
- Determinism and replay: semantic validation report is D0 under the exact view/graph/validation-derivation/checker-build/checker-protocol/checker-schema/profile/rules/cards/limits; RPT.004 keeps host observations separate and RPT.009 records any admitted replay relation.
- Falsifiers and Gauntlet: omit or swap validation derivation, checker build, checker protocol/schema, or per-rule implementation identity; infer one layer's pass from another; aggregate pass with a failed/unrun rule; non-transitive pass laundering; profile or rule-set swap; stale view/graph; card withdrawal; partial-as-whole coverage; missing unsupported class; invented external comparison; cancellation; and checker disagreement.
...
- Dependencies and consumers: depends on DOC.007/DOC.009, IMM.004, and FDN.001/FDN.008/FDN.009/FDN.012/FDN.015; RPT.001/RPT.004/RPT.005/RPT.009, IMM.016, FUZ.009, and the product/close gates consume it.
- Migration and withdrawal: validation derivation grammar, checker build/protocol/schema, profile, rule-set, per-rule implementation/algebra, location schema, or layer-definition change versions the report; withdrawn cards lapse affected results without deleting historical reports.
...
- Rationale: a typed report preserves non-transitive validation evidence while making every proper-domain and checker identity, profile, rule, coverage, unsupported class, and C1 comparison absence machine-checkable.
...
- Inputs and identity domain: contract ID/version, lifecycle, owner layer, input domain, preconditions, budgets, authority, outcomes, evidence, determinism, falsifiers, surfaces, dependencies, exact §5 grammar-expanded prerequisite IDs, their mechanically inverted direct-consumer index, separately indexed assurance targets and guarded state-transition consumers, cards, no-claims, typed `ConsequenceAxisRecord`, operation-local `ClaimAlgebraRecord`, exact shell §27 content root/section version, applicable G0–G7/G6.1 tier rows, applicable §27.1 falsifier-family rows, and §27.4 release-matrix row or typed not-applicable disposition.
- Preconditions: every presentation field validates; every required consequence axis has an exact typed value or `NotApplicable(reason_code, authority)` plus source-field link; every operation has a registered claim algebra and exact versioned shell §27 tier/falsifier/release assignment; a local C1 projection cannot replace or narrow that authority; every dependency token expands uniquely against the closed catalog; the direct-consumer index equals the exact inverse of the expanded prerequisite list; bare-family/prose prerequisites are rejected; dependencies exist and layer direction is legal; each admitted §8.1 L8-assurance-to-L7 flow uses the explicit §5 guarded injected-data form and has no producer import; card slots are present even when pending.
- Card slots: `SC.C1.IMM.CONTRACT-REGISTRY.001` — `PENDING-LICENSED-SOURCE`.
- Budgets: contracts, fields, dependency edges, Gauntlet tier/falsifier/release assignments, schema bytes, canonicalization, diagnostics, and generated projections are bounded.
...
- Outputs and outcomes: canonical registry root, per-contract records with typed consequence axes and claim algebras, exact sorted prerequisite-edge index, its mechanically inverted direct-consumer index, separate assurance-target and guarded state-transition indexes, exact shell §27 root/version plus tier/§27.1-falsifier/§27.4-release assignments, lifecycle/migration links, and validation findings.
- Diagnostics: duplicate ID, missing field/axis/card/falsifier/surface, missing or stale shell §27 root/version, omitted G7/G6.1 or applicable §27.1/§27.4 assignment, unauthorized local taxonomy replacement, inferred or unjustified not-applicable axis, missing/unknown claim algebra, unknown/bare/ambiguous dependency token, non-inverted direct-consumer index, range expansion outside one family, illegal or prose-only dependency, unguarded injected data, producer import disguised as data injection, unknown outcome, lifecycle contradiction, and orphan contract.
- Evidence and provenance: registry binds plan/Constitution authority references, exact shell §27 content root/section version and assignment rows, generator/build identity, source records, and review/change record.
...
- Falsifiers and Gauntlet: duplicate/missing fields or axes, omitted/stale shell §27/G6.1/G7/§27.1/§27.4 binding, local tier replacement, unjustified not-applicable value, unknown algebra/composition, dependency cycles, reverse-index mismatch, layer violation, prose ingested as a direct consumer, injected-data relation that imports or invokes its producer, surface widening, and migration cases.
- Surfaces: claim/capability/report generation, repository gates, documentation, checker, and close gate.
- Dependencies and consumers: depends on FDN.001–FDN.018 and IMM.001; IMM.003–IMM.010/IMM.012/RPT.006 consume it.
- Migration and withdrawal: material contract change mints a version/migration/claim-impact link; any shell-taxonomy, falsifier-pairing, or release-matrix-validity change follows the §34.9 kernel-touch route; every prior record is immutable.
...
- Inputs and identity domain: contract registry, each operation's `ClaimAlgebraRecord`, active profiles, report schemas, surface manifest, evidence/gate requirements, lifecycle/expiry/withdrawal state, and claim vocabulary.
- Preconditions: each claim has an exact contract/profile/input domain, registered required-input/scope/approximation/refusal algebra, evidence requirement, falsifier, outcome scope, and no-claim boundary; unknown composition fails closed.
...
- Outputs and outcomes: canonical claim registry, contract-to-claim/algebra links, profile/lifecycle/expiry/withdrawal fields, composition/refusal records, and rejected orphan assertions.
...
- Falsifiers and Gauntlet: invented heading, missing/unknown algebra, deleted API, renamed contract, wider docs/CLI claim, expired/withdrawn row, and composition mismatch.
...
- Inputs and identity domain: RPT contract records, field/type registry, identity obligations including the RPT.010 validation derivation and checker build/protocol/schema identities, outcome/coverage algebra, diagnostic registry, migration rules, and generator.
- Preconditions: every report family names exact proper-domain identities and validates required fields before example generation; RPT.010 schema/example generation requires the role-labeled validation `DerivationId`, checker build identity, checker protocol/schema identities and versions, and every per-rule implementation identity/version.
...
- Outputs and outcomes: schemas, canonical encoders, validators, migrations, synthetic examples, schema-to-contract/identity links, and ValidationReport fixtures that bind every mandatory derivation/checker/per-rule identity.
- Diagnostics: missing `SourceRootId`/`RevisionGraphId`/`DocumentViewId`/`DerivationId`/`ExpectedStateId` where mandated, missing ValidationReport checker build/protocol/schema or per-rule implementation identity, contradictory coverage, unsafe field, and stale example.
...
- Falsifiers and Gauntlet: remove each mandated identity in turn, swap validation derivation or checker build/protocol/schema identity, omit a per-rule implementation version, role-label mismatch, ambiguous optional field, stale hand edit, malformed example, and producer/checker round trip.
...
- Migration and withdrawal: schemas/examples/migrations regenerate together for every validation-derivation or checker protocol/schema change; hand-patching generated examples is forbidden.
...
- Inputs and identity domain: contract surface records, generated API inventory from source/build metadata once code exists, schema bindings, feature matrix, concrete-crate inventory, each concrete crate's `CONTRACT.md`, build identity, FDN.015 `StabilityActivationRecord` set, prior admitted surface roots, and applicable FUZ.014 compatibility-run receipts.
- Preconditions: D7 is human-ratified; API inventory generation itself is reproducible and scoped to the admitted build/features. Compatibility comparison is required only for a surface whose stability record is active and must bind its prior admitted root; `NoStabilityPromise` cannot be reported as a compatibility pass. C1's general Rust facade remains `NoStabilityPromise`; selected schema, diagnostic, and CLI rows retain their declared major-version scopes. **PROPOSED default — awaiting ratification (D7):** an exact stable runtime/release toolchain and a separate date-pinned nightly used only by assurance lanes; no exact version is inferred here.
...
- Outputs and outcomes: per-contract API existence/signature/feature/surface match; per-concrete-crate `CONTRACT.md` existence and coherence with owned layer, role, public surface, invariants, dependencies, safety section, and contract IDs; per-active-scope compatible/breaking/indeterminate comparison against the prior admitted root; explicit no-promise state for inactive scopes; missing/extra/unbound items; and exact build coverage.
...
- Falsifiers and Gauntlet: delete/rename API, remove error variant, claim compatibility without activation/prior root/run receipt, hide a breaking active-scope change, manufacture a Rust-facade promise, remove or stale a concrete crate's `CONTRACT.md`, contradict its owned contracts/layer/dependencies/safety section, feature-gate mismatch, stub/heading-only declaration, stale generated inventory, and checker mutation.
- Surfaces: repository gate, drift audit, claim/capability admission, cycle-close evidence, and trial record.
- Dependencies and consumers: depends on IMM.002–IMM.004, FDN.015/FDN.016, FUZ.014, and ratified toolchain; IMM.012 and close gate consume it.
...
- Identity, lifecycle, layer: active C1 L7 repository-gate contract; it validates runtime dependency direction, package role, contract declarations, the prerequisite DAG and its mechanically inverted direct-consumer index, the separately guarded state-transition graph, and candidate-bound dependency hygiene.
- Inputs and identity domain: exact candidate source/build/toolchain/lockfile roots; package/module and resolved feature/target dependency graphs; §5 grammar-expanded prerequisite DAG and generated reverse index; separately labeled guarded state-transition graph with injected-data classifications and producer-import prohibitions; layer/role metadata; contract ownership; every concrete crate's `CONTRACT.md`; feature/dependency manifests; allowlist decisions; phase labels/guards; an admitted date-pinned local dependency-advisory snapshot with provenance/availability; duplicate-version policy and explicit allowlist decisions; and build identity.
- Preconditions: every concrete crate has an authenticated `CrateBoundaryJustification` proving at least two Rev 7 §30.1.1 criteria; a criteria violation fails and missing/uncertain evidence is indeterminate. Only versioned kernel change control may alter the criterion law. D5, D7, and D-005 routing are ratified as applicable. The candidate hygiene lane has an exact lockfile/build/feature/target graph and a locally supplied, provenance-recorded advisory snapshot; a missing, stale, unavailable, or unsupported snapshot yields indeterminate rather than clean. **PROPOSED default — awaiting ratification (D5):** MIT OR Apache-2.0 with a dependency-license rider, subject to counsel; D-005 still leaves runtime policy and every provider symbolic. **PROPOSED default — awaiting ratification (D7):** exact stable for runtime/release plus date-pinned nightly for assurance findings only.
...
- Outputs and outcomes: illegal upward edge/cycle, prerequisite or reverse-index mismatch, state-transition phase/guard violation, transition mislabeled as prerequisite, injected data not already minted, injected-data relation importing or invoking its producer, undeclared dependency, role or crate-contract mismatch, unratified provider, boundary-budget pass/fail/indeterminate with the exact qualifying criteria per crate, and separate results for each graph and concrete boundary; plus one immutable `DependencyHygieneResultId` with per-advisory affected path/status/disposition, duplicate-version groups and allowed/refused/indeterminate decisions, exact feature/target coverage, snapshot freshness/availability, and clean/fail/indeterminate aggregate state.
- Diagnostics: graph class, edge path, phase/guard, injected artifact and producer-import status, owner layers/roles, violated law, feature/build context, advisory/snapshot identity and freshness, duplicate versions and paths, allowlist decision, missing or incoherent crate contract, and decision block are explicit.
- Evidence and provenance: report binds exact candidate/source/build/toolchain/lockfile roots, resolved dependency graphs, contract registry, every `CrateBoundaryJustification` and its qualifying criteria, advisory-snapshot identity/date/provenance/availability, every advisory and duplicate-version disposition, allowlist decision IDs, and checker version; later clean snapshots do not erase historical findings.
...
- Falsifiers and Gauntlet: inject upward edge/cycle, inject a prerequisite cycle, corrupt the reverse index, merge a guarded recovered-mode or injected-data transition into the prerequisite DAG, reverse or remove a transition phase/guard, import or invoke an injected artifact's producer, admit a crate with fewer than two criteria, forge or reuse a boundary justification, remove or contradict a crate `CONTRACT.md`, add runtime dependency on tool/lab, hide a feature dependency, select an unratified crypto provider, seed an advisory, omit or stale the advisory snapshot, split an unreviewed duplicate version, forge an allowlist disposition, change a feature/target path, and omit metadata.
...
- Dependencies and consumers: depends on IMM.002/IMM.003/IMM.006 and the versioned candidate CLI command/help declarations; in state transition `candidate-manifest-finalized -> claim-link-check-enabled`, additionally consumes RPT.006 only when its immutable candidate-manifest root is supplied as injected data. The manifest root and CLI declarations are registry inputs; no RPT.006 producer implementation or CLI surface is imported or invoked. IMM.012 and CLI.008 consume the check result.
...
- Inputs and identity domain: canonical structured registries, generated files/examples/docs/help/manifests, adjudicated behavioral goldens, typed `GoldenChangeRecord`, the rights-admitted versioned Arlington source artifact and source-vintage identity when present, its tool-only importer and generated validation-rule data, generator versions, provenance headers, and expected roots.
- Preconditions: generated outputs carry source/generator identity; manually maintained prose does not claim generated authority. A behavioral golden change requires semantic reason, upstream-coupling inventory, independent oracle result, old/new correctness review, authorized deliberate re-freeze, and exact affected roots; ordinary bulk regeneration is invalid. Derived echoes may regenerate deterministically from their canonical root and are not behavioral-golden adjudication. Arlington import is unavailable until source rights and the exact source artifact are admitted; the importer runs only as a tool and DOC.007 remains a fail-closed non-authority bridge.
...
- Outputs and outcomes: exact derived-echo match; stale/mutated/missing/unattributed echo; source-root mismatch; schema-example drift; accepted/refused/indeterminate behavioral-golden re-freeze with `GoldenChangeRecordId`; protected-data incident; and—only after rights admission—a versioned Arlington generated-rule-data echo bound to importer identity and exact source vintage.
...
- Falsifiers and Gauntlet: edit generated example, omit identity field, stale CLI help, duplicate hand table, bulk-regenerate a behavioral golden, omit semantic reason/coupling/oracle/old-new review/re-freeze authority, confuse a derived echo with an adjudicated golden, wrong provenance, Arlington source-vintage or importer mismatch, runtime use of importer data as standalone authority, unauthorized source input, and restricted-content injection.
- Surfaces: repository gate, docs/help/schema generation, capability manifest, drift audit, and close gate.
- Dependencies and consumers: depends on IMM.002–IMM.005, DOC.007, the admitted source-rights record when Arlington import is present, and versioned CLI projection declarations/fixtures; in state transition `projection-and-manifest-artifacts-finalized -> generated-drift-check-enabled`, additionally consumes RPT.005/RPT.006 only when their exact immutable artifacts are supplied as injected data. No RPT.005/RPT.006 producer implementation is imported or invoked; the active CLI surfaces remain downstream consumers. IMM.012 and CLI.008 consume the check result.
...
- Inputs and identity domain: immutable RPT.006 candidate-manifest root, canonical/generated roots, API/dependency/claim/coverage/source scans, schema migrations, every changed behavioral golden's IMM.010 `GoldenChangeRecordId`, build, findings/dispositions, and audit protocol.
- Preconditions: candidate manifest was finalized before audit; every component check ran over that unchanged manifest and exact candidate artifact/build; each changed behavioral golden has an accepted re-freeze record while derived echoes follow their separate deterministic regeneration law; findings are graded with stable evidence locations.
...
- Diagnostics: missing/stale component, candidate mismatch, missing/refused/indeterminate golden re-freeze record, derived-echo/golden confusion, unresolved Grade A, invalid waiver, source incident, and auditor self-failure.
...
- Falsifiers and Gauntlet: seed deleted API, layer cycle, orphan claim, stale schema, changed behavioral golden without its full re-freeze record, ordinary bulk regeneration, pending card, prohibited context marker, invalid waiver, and cancelled component.
- Surfaces: repository gate, RPT.008 capability-admission envelope, trial record, morning report, and evidence package.
- Dependencies and consumers: depends on IMM.006–IMM.011 and IMM.015; in state transition `candidate-manifest-finalized -> drift-audit-enabled`, additionally consumes RPT.006 only when its unchanged immutable candidate-manifest root is supplied as injected audit data. No RPT.006 producer implementation or L8 surface is imported or invoked; RPT.008, IMM.016/IMM.019, and the C1 close gate consume the audit result.
...
- Inputs and identity domain: exact strict/recovery/enrichment/validation/replay/capability-admission report roots including RPT.009 and RPT.010 when applicable, applicable schema-valid RPT.005 projection artifacts, contract/claim/schema/card roots, FDN.010 availability records, checker protocol, every corroborating observation’s general `OracleLineageId`, checker-specific pre-run/final lineage roots, FDN.017 `HostOutputCapabilityId`, no-claims, disclosure policy, and canonical package protocol.
- Preconditions: D1/D2 and D4 plus disclosure policy are ratified; all inputs are immutable and schema-valid; each artifact's availability/materialization path validates under FDN.010; the requested publication class is admitted, narrowed, or refused by FDN.017. **PROPOSED defaults — awaiting ratification (D1/D2/D4):** the coupled SHA-256/deterministic-CBOR package and the staged checker-isolation profile; neither is activated by this plan.
...
- Outputs and outcomes: canonical D0 semantic package root, D1 serialized package where inputs are pinned, artifact availability map with materialization/recomputation/refusal receipts, FDN.017 `HostPublicationReceiptId`, checker target protocol, and exact no-claims.
...
- Evidence and provenance: package binds exact report/registry/artifact roots, availability/materialization receipts, producer protocol/build, disclosure policy, FDN.017 capability/publication roots, limits, and construction diagnostics.
- Determinism and replay: unsigned package semantics target D0 Structural under exact inputs/protocol; canonical serialized package bytes target D1 Serialized and are separately disclosed and checked.
...
- Dependencies and consumers: depends on FDN.001–FDN.017, IMM.002–IMM.005, IMM.014, and IMM.020; in state transition `applicable-report-roots-finalized -> private-package-assembly-enabled`, additionally consumes RPT.001–RPT.005/RPT.007–RPT.010 only when their exact immutable roots or projection artifacts are supplied as injected assembly data. The RPT.006 candidate-manifest producer artifact is outside this direct assembly relation, and no report-producer implementation or L8 surface is imported or invoked; IMM.013 independently consumes raw package output rather than producer objects, while CLI.009, IMM.019, and the close gate consume package/checker results.
...
- Inputs and identity domain: `ProductionLedgerBootstrapId`, versioned event schema, writer identity and verified model route where available, event class, attempt/branch relation, opaque artifact/build/toolchain/protocol roots available at that moment, applicable IMM.021 `EvaluationProtocolCommitmentId` and amendment head for trial/probe events, payload, evidence status, retrospective flag, prior writer-shard digest, and disclosure policy.
- Preconditions: D1/D2/D6 are human-ratified; IMM.018 finished before implementation candidate work; writer-shard ownership is exclusive; retrospective status is structurally mandatory. A campaign trial/probe start event additionally binds an independently custodied IMM.021 protocol that predates candidate generation or is marked exploratory/claim-ineligible. **PROPOSED default — awaiting ratification (D6):** hash-chained append-only JSONL per-writer shards as source of truth plus a regenerable derived index.
...
- Outputs and outcomes: `ProspectiveEventRecordId` and new shard head, bound evaluation-protocol/amendment state where applicable, duplicate-idempotent observation where schema permits, or typed refused/cancelled/indeterminate storage outcome. Derived indexes are disposable and never source of truth.
...
- Evidence and provenance: each event binds bootstrap, prior/new shard heads, writer, event/attempt relation, exact then-available roots, applicable evaluation-protocol/amendment root, evidence status, disclosure result, and append receipt; later final roots cannot be backfilled into it.
...
- Falsifiers and Gauntlet: append before bootstrap, trial start without a predating protocol or explicit exploratory state, protocol-root substitution, concurrent writers, torn line, prior-digest corruption, retrospective omission, missing failed attempt, secret injection, index/source disagreement, crash around acknowledgement, and chain-head substitution.
...
- Dependencies and consumers: depends on IMM.018 and ratified D1/D2/D6 only; in state transition `prospective-ledger-open -> protocol-bound-trial-event-enabled`, additionally consumes IMM.021 only when a campaign trial/probe event supplies the predating evaluation protocol. IMM.019, §9.7, §11.2, and §11.5 consume the prospective stream.
...
- Inputs and identity domain: bootstrap ID, interval start/end heads, IMM.021 `EvaluationProtocolCommitmentId` and amendment chain, versioned `TrialAggregationProtocolId`, every prospective event/attempt/failure/retry/intervention, final build/toolchain/dependency/card/report/capability/audit/checker/fuzz/package roots, typed `HeldOutProbeProtocolId`/`HeldOutProbeId`/`HeldOutProbeResultId`, availability, gate dispositions, missing telemetry, and no-claim state.
- Preconditions: the bootstrap and independently custodied evaluation protocol predate candidate generation; all writer shards verify from genesis to end heads; the aggregation protocol fixes population, denominator, outcome order, missingness, percentile/median, favorable-selection count/rule, route treatment, and stopping before result inspection; final roots are immutable and refer to recorded events or explicit retrospective gaps; the held-out probe proves sealed-pool membership, independent selection, candidate binding, access-budget accounting, terminal result retention, reveal demotion, and replacement state; D1/D2/D6 remain the ratified protocols.
...
- Outputs and outcomes: immutable `CycleTrialRecordId`, protocol/amendment roots, interval/shard-head set, versioned aggregation with population/denominator, outcome taxonomy, success rate, median/percentiles, favorable-selection rule/count, retries/restarts, routes, cost, interventions, representative failures, every discarded candidate, typed held-out probe result, final root matrix, missing/retrospective gaps, gate states, artifact availability, and seal receipt.
- Diagnostics: missing/stale/post-candidate evaluation protocol, unauthorized amendment, aggregation denominator/order/missingness/stopping drift, omitted attempt/route/failure, chain break, missing writer shard/event, retrospective gap, root without event lineage, probe pool/selection/access/reveal/replacement mismatch, stale build, conflicting gate state, unavailable artifact, disclosure failure, unratified protocol, and seal transaction failure.
- Evidence and provenance: record binds bootstrap, IMM.021 protocol/custody/amendment roots, aggregation protocol, verified shard/event/all-attempt roots, typed held-out probe roots, final RPT.008/IMM.012/IMM.020/FUZ.011/FUZ.015/FUZ.016 and other scoped roots, human dispositions, sealer identity, limits, and construction receipt.
...
- Falsifiers and Gauntlet: omitted failed attempt/route/discarded branch, denominator or stopping drift, favorable-only selection, post-candidate protocol, overwritten amendment, probe selection/result/reveal mismatch, post-hoc inserted event, head substitution, final root without event, stale audit/candidate pair, missing checker miss, fuzz-baseline mismatch, secret injection, cancellation, and alternate merge-order test.
...
- Dependencies and consumers: depends on IMM.017/IMM.018/IMM.021, RPT.004/RPT.007/RPT.008, IMM.012–IMM.016/IMM.020, FUZ.011–FUZ.016, and final gate/probe/card roots; §11.5 and the next-cycle archaeology record consume it.
...
- Identity, lifecycle, layer: active only after human/steward action as a pre-run C1 L7 governance contract; it describes one immutable protocol and amendment chain without performing custody or commitment.
- Inputs and identity domain: `EvaluationProtocolId`, canonical protocol schema/version/root, claim/reference class, artifact boundary, denominator/sampling frame, held-out access budget, metric and uncertainty rules, multiplicity, missingness, adjudication, stopping/reveal law, source/corpus manifests or blinded commitments, evaluator/oracle lineage, rights/conflicts, custody evidence, and authorized amendments.
- Preconditions: the authorized human/steward fixes the protocol before candidate generation, unblinding, or result inspection; commitment custody is independent of the candidate-producing writer; a material amendment creates a new protocol ID and fresh-data requirement.
- Card slots: `SC.C1.IMM.EVALUATION-PROTOCOL.001` — `PENDING-LICENSED-SOURCE`; this project-law slot contains no external standard semantics.
- Budgets: protocol fields, manifests, amendment links, custody/witness records, access-policy entries, canonicalization, diagnostics, and artifact bytes are bounded.
- Cancellation: interrupted construction or custody acknowledgement yields no admitted commitment; a private draft cannot authorize trial evidence.
- Authority and secrets: only authenticated human/steward roles may commit, amend, reveal, or attest custody; blinded corpus details, rights constraints, and protected identities follow disclosure policy.
- Outputs and outcomes: `EvaluationProtocolCommitmentId` with protocol root, custody/witness evidence, commit/reveal state, claim ceiling, authorized amendment head, superseded IDs, and fresh-data requirement; otherwise draft, refused, unavailable, or indeterminate state.
- Diagnostics: late commitment, mutable/private-only draft, missing custodian, claim/reference/denominator/metric/missingness/stopping gap, unauthorized amendment, reveal mismatch, rights conflict, and stale protocol.
- Evidence and provenance: binds protocol schema/content root, author/approver/custodian roles, commitment observation, source/corpus or blinded roots, lineage/conflicts, amendment chain, reveal observation, and availability.
- Determinism and replay: the protocol semantic root is D0 Structural and any canonical serialized commitment is D1 Serialized under ratified D1/D2; custody time/observation remains separate evidence.
- Falsifiers and Gauntlet: protocol after candidate/result, private mutable document represented as committed, custodian alias, omitted denominator/missingness/stopping, post-result amendment, old/new result splice, reveal-byte mismatch, and forged witness.
- Surfaces: pre-C1 campaign preflight, IMM.017 trial/probe events, FUZ.013–FUZ.016 protocol mapping, IMM.019 seal, and campaign close gate; no engine runtime imports it.
- Dependencies and consumers: depends on human-established evaluation custody and ratified D1/D2 only; IMM.017/IMM.019, FUZ.013–FUZ.016, and the campaign close gate consume it.
- Migration and withdrawal: material change mints a successor and requires fresh affected evidence; old protocols/results remain immutable and scoped.
- No-claim boundary: this plan defines an interface only; it makes no external commitment, runs no evaluation, opens no corpus, and establishes no result.
- Rationale: prospective trial meaning must be fixed independently before outcomes can influence its rules.

#### `C1.CC.IMM.022@1` — Human-owned vulnerability-response policy status

- Identity, lifecycle, layer: active only after authenticated human adoption as a C1 L7/L8 gate-status contract; it contains policy status and linkage, never policy prose.
- Inputs and identity domain: `VulnerabilityResponsePolicyStatusId`, policy artifact identity/version, authorized issuer/reviewer roles, draft/adoption/lapse/withdrawal state, review record, effective scope/date, intake/triage/embargo/advisory-format coverage flags, claim-withdrawal linkage, and availability.
- Preconditions: only the human-established security-governance role may issue or change status; every required coverage flag links to the separately reviewed policy artifact; missing/unreviewed/unavailable content cannot be represented as adopted.
- Card slots: `SC.C1.IMM.VULNERABILITY-STATUS.001` — `PENDING-LICENSED-SOURCE`; this project-law slot contains no policy body or external standard semantics.
- Budgets: status fields, role/review records, lifecycle links, evidence references, diagnostics, canonicalization, and artifact bytes are bounded.
- Cancellation: interrupted status review yields no adopted state; the prior status remains historical and current customer admission stays blocked as applicable.
- Authority and secrets: status handling grants no channel, contact, publication, advisory, embargo, or incident authority; protected policy/incident details stay outside the public status record.
- Outputs and outcomes: authenticated `VulnerabilityResponsePolicyStatusId` in draft/adopted/lapsed/withdrawn/unavailable/indeterminate state with exact coverage flags, evidence root, reviewer roles, lifecycle links, and customer-gate disposition.
- Diagnostics: unknown issuer, missing review, stale version, absent coverage flag, policy/status root mismatch, unauthorized adoption, lapse, unavailable artifact, and claim-withdrawal linkage gap.
- Evidence and provenance: status binds policy artifact identity, issuer/review roles, adoption/lapse records, coverage review, evidence availability, gate scope, and lifecycle history without copying policy content.
- Determinism and replay: structured status is D0 Structural under exact authority and policy roots; governance judgment remains attributed and time-scoped.
- Falsifiers and Gauntlet: string-only token, forged issuer, adopted-without-review, missing required flag, stale policy substitution, lapsed status admitted, copied policy prose, or external action inferred from status.
- Surfaces: capability-admission row, customer-availability gate, claim lifecycle/withdrawal record, trial seal, and capability discovery; no engine semantic layer imports it.
- Dependencies and consumers: depends on the human-established security-governance authority only; RPT.008, IMM.015/IMM.019, CLI.008, and product-gate row 15 consume it.
- Migration and withdrawal: policy/status version or authority change creates a linked successor; lapse/withdrawal preserves prior evidence and blocks affected customer-facing admission.
- No-claim boundary: a status record is not policy content, a public channel, incident handling, legal advice, publication, or evidence that any external action occurred.
- Rationale: a gate-bearing human decision needs authenticated versioned state without drafting or operationalizing the policy in plan space.
...
- Inputs and identity domain: semantic report or registry, requested format/schema version, disclosure policy, stdout/stderr FDN.017 `HostOutputCapabilityId` values and requested publication classes, color/locale policy for human rendering, and limits.
- Preconditions: source object is schema-valid; machine output is not mixed with human diagnostics on the same channel unless framed explicitly; each requested publication class is admitted, narrowed, or refused by its exact host-output capability.
...
- Outputs and outcomes: schema-tagged JSON, framed JSONL, or deterministic human rendering plus semantic report ID, projection receipt, and exact FDN.017 publication receipt for each used channel.
...
- Evidence and provenance: receipt binds semantic input root, projection version, disclosure, host-output capability/publication roots, output identity, and losses/redactions.
- Determinism and replay: canonical machine bytes are D1 Serialized where declared; their decoded semantic body is D0 Structural. Terminal rendering capabilities remain recorded run inputs.
...
- Dependencies and consumers: depends on RPT.005, FDN.008/FDN.015/FDN.017; CLI.001–CLI.004/CLI.008–CLI.010 consume it.
...
- Outputs and outcomes: exact contract/card/profile/lifecycle/no-claim rows, including structured source/dialect/filter/decryption/report/CLI/font/color-ICC/validation-profile/determinism/host-adapter categories and their `Available`/`Absent`/`Disabled`/`DecisionBlocked`/`Unsupported`/`Unavailable` states; decision blocks, report schema, candidate-manifest root, audit-receipt root, admission-envelope root/status, and running-build identity.
...
- Inputs and identity domain: one admitted source/open artifact, exact source-backed `RevisionGraphId` and `DocumentViewId` selector, role-labeled validation `DerivationId`, admitted checker build identity and checker protocol/schema identities and versions, active C1 validation profile/profile version, rule-set identity/version, per-rule implementation identities/versions, requested layer/rule projection, limits, disclosure policy, and output sink.
- Preconditions: source/open and exact view/graph admission succeed; validation-derivation/checker-build/checker-protocol/checker-schema identities are present and mutually bound; every executed normative rule has an active reviewed card and exact implementation identity/version; the requested profile contains only C1 structural-consistency, declared-constraint-consistency, and independently sourced model-check layers. Unsupported, unavailable, pending, or unrun rules remain explicit and cannot pass.
...
- Outputs and outcomes: emits the immutable RPT.010 `ValidationReportId` binding the exact validation derivation, checker build/protocol/schema, profile/rule-set, and per-rule implementation identities; separate layer 1–3 summaries and per-rule outcomes/coverage/locations; unsupported classes; exact machine outcome; projection receipt; and CLI.007 exit mapping.
- Diagnostics: usage/selector/profile/rule-set errors remain separate from validation outcomes; missing or mismatched validation derivation/checker build/protocol/schema/per-rule identity, pending card, unsupported layer/profile, unavailable artifact, cancellation, output failure, and rule-engine fault retain distinct codes.
- Evidence and provenance: command receipt binds source/open root, exact graph/view/validation-derivation/checker-build/checker-protocol/checker-schema/profile/rule/card and per-rule implementation identities, RPT.010 root, requested projection, limits, disclosure, build/toolchain, used capabilities, and run envelope.
- Determinism and replay: semantic validation is D0 under the exact view/graph/validation-derivation/checker-build/checker-protocol/checker-schema/profile/rules/cards/limits; CLI.006 owns projection and RPT.004/RPT.009 retain observation/replay boundaries.
- Falsifiers and Gauntlet: omit or swap validation derivation/checker build/protocol/schema/per-rule implementation identity, infer one layer's pass from another, promote an unrun/pending rule to pass, request an unsupported standardized profile, use a stale graph/view, hide partial coverage, cancel, break output, leak secret/location data, fail schema round trip, or disagree between inspect and validate RPT.010 roots.
...
- Migration and withdrawal: command/validation-derivation/checker protocol/schema/profile/rule/projection semantics change versions the surface; a withdrawn card lapses only affected results and never rewrites historical reports.
...
- Rationale: the active C1 validation report needs its own deterministic §25.3 command with every mandatory derivation/checker identity and without importing later standardized-profile validation.
...
- Inputs and identity domain: operation scenario including CLI.001–CLI.010 request/option parsing, saved-artifact admission, projection/framing, FDN.005 `FallibleAllocationLease`, FDN.017 host-output capability/publication phases, and interrupt paths; checkpoint schedule, allocation/host/resource fault script, worker/cache schedule, source/credential partitions, exact command/candidate build, and limits.
- Preconditions: target operation exposes registered checkpoints, resource classes, reservation-to-allocation boundary, and host publication states; concurrency and host-capability models match the admitted implementation.
...
- Outputs and outcomes: panic/deadlock/livelock/race/leak/double-refund/allocation-reconciliation/publication-class/partial-publication/nondeterminism/isolation finding with replay relation.
...
- Falsifiers and Gauntlet: every checkpoint class; CLI unknown/duplicate/oversized option and hostile value; unchecked saved package; schema mismatch; JSON/JSONL terminal framing; output truncation; broken pipe; redaction; interrupt/reconciliation; sibling reservations/refunds; allocation failure/refund/cleanup; short write; quota/space exhaustion; flush and data/directory-durability failure; symlink/path replacement; no-clobber race; permission/ownership change; cross-filesystem rename; commit/abort and post-visibility failure; cache insert/evict; diagnostic collector; source change; credential crossing; and thread count.
- Surfaces: all C1 targets, cancellation gate, panic gate, drift evidence, and trial record.
- Dependencies and consumers: depends on FUZ.001; targets FDN.004–FDN.014/FDN.017/FDN.018, CLI.001–CLI.010, and every transactional/cache-bearing contract; FUZ.011/FUZ.012/FUZ.014 consume findings.
...
- Inputs and identity domain: original finding/witness, target/build, minimization transforms, semantic predicate, cards/contracts, sibling-site search, fix evidence, affected behavioral goldens, IMM.010 `GoldenChangeRecordId` where a re-freeze is proposed, consumed compute, consumed human attention, disclosure classes, and retention policy.
- Preconditions: minimization preserves the triggering property under exact build/limits; protected data is not declassified. A changed behavioral golden has semantic reason, upstream-coupling inventory, independent oracle, old/new correctness review, and deliberate re-freeze authority; ordinary bulk regeneration cannot close a finding.
...
- Outputs and outcomes: minimized witness, regression property/fixture link, violated assumption, affected contracts, sibling search, fix evidence, accepted/refused/indeterminate golden-change link where applicable, tombstone/replacement rule, exact consumed compute and human attention with disclosure class/availability, and limits.
- Diagnostics: non-reproducing candidate, property drift, privacy/right block, minimization cap, stale build, missing sibling search, missing or invalid golden re-freeze record, bulk-regeneration attempt, and false closure.
...
- Falsifiers and Gauntlet: minimizer deleting trigger, secret leak, one-input-only fix, missing contract update, behavioral golden changed without all required evidence, derived-echo/golden confusion, stale corpus, repeated tombstoned heuristic, and cancellation.
...
- Inputs and identity domain: item manifests, partition/steward commitments, IMM.021 `EvaluationProtocolCommitmentId`, `HeldOutProbeProtocolId`, sealed-pool membership, independent selection rule, access policy/log, submission/feedback budget, exact candidate binding, reveal rule, replacement reserve, and disclosure policy.
- Preconditions: held-out seal and evaluation/probe protocols predate implementation access and candidate generation; unavailable steward/seal/protocol is recorded and cannot be reconstructed; selection is independently recorded before reveal.
...
- Outputs and outcomes: partition registry, `HeldOutProbeProtocolId`, selected `HeldOutProbeId`, seal/pool/selection/access/submission/candidate-binding record, aggregate feedback as predeclared, reveal/demotion state, and fresh replacement requirement.
- Diagnostics: pre-seal or pre-protocol access, late/unauthorized selection, candidate mismatch, unauthorized reveal, feedback excess, provider-retention unknown, missing steward, partition alias, replacement gap, and confidential commitment risk.
- Evidence and provenance: record binds IMM.021 commitment, steward/probe protocol, pool/item commitments and availability, selection receipt, access log, exact artifact submission, feedback, reveal/demotion, and replacement events.
- Determinism and replay: evaluation record is D0 for logged events/protocol; hidden artifact bytes may remain unavailable.
- Falsifiers and Gauntlet: seeded access leak, cache/embedding leak, repeated adaptive feedback, missing/changed seal or protocol, post-reveal selection, candidate substitution, unauthorized dashboard, missing demotion, and missing replacement after reveal.
- Surfaces: held-out C1 probe, corpus registry, trial record, close gate, and future cycle evidence.
- Dependencies and consumers: depends on FUZ.001, IMM.021, human day-zero sealing, commitment substrate, FDN.008/FDN.015; FUZ.014–FUZ.016 and the C1 campaign gate consume it.
...
- Inputs and identity domain: FUZ.016 baseline, lane/target/run-class IDs including general suite mutation/fault-seeding, `G3SubsetRun`, `HeldOutProbeRun`, `ReproducibleBuildRun`, and `StabilityCompatibilityRun`; IMM.021 protocol where required; exact candidate and guidance-instrumented or paired-build/toolchain/dependency roots; corpus/fixture/seed roots and rights; G3-subset protocol/right/custody/external-artifact/configuration/request identities; held-out probe identities; stability activation/prior-surface root; target configuration; versioned guidance or equivalence schema; initial queue/corpus state; semantic/resource limits; schedule/fault profile; declared mutation/fault seeds with expected detection class; worker identity; general `OracleLineageId`; pre-run manifest root; and prospective-ledger event IDs.
- Preconditions: baseline admits the lane; target/corpus/card/rights coverage is active; coverage-guided lanes bind exact instrumentation and feedback policy before launch; G3-subset requires a precommitted rights/custody protocol and exact external black-box artifact/configuration; held-out probe runs bind FUZ.013 selection/access state; reproducible-build runs bind independently produced build roots, pinned source/toolchain/lock/features/target inputs, separated environments, and artifact-equivalence rule; stability runs require an active FDN.015 scope and prior admitted surface root, while no-promise scopes remain unavailable; runtime and crypto-provider choices remain symbolic until human ratification; start event is prospectively appended through IMM.017 before execution.
...
- Outputs and outcomes: immutable `AssuranceRunManifestId` and `AssuranceRunReceiptId`, start/end event links, run class, executed scope, terminal state, finding/witness IDs, fired limits, missing work, environment/build roots, `OracleLineageId`, and explicit pass/fail/indeterminate/unavailable contribution. G3-subset receipts additionally bind protocol/right/custody, external artifact/configuration, request/result identities, disagreement/minimization state, and unavailable reason. Held-out runs emit `HeldOutProbeResultId` with selected probe/candidate/access/terminal/reveal state and retain favorable or unfavorable results. Reproducible-build receipts bind both build roots/environments, equivalence rule, discrepancies, and nondeterminism classification. Stability receipts bind activation/prior/current surface roots and compatible/breaking/indeterminate result. General mutation/fault-seeding and coverage-guided receipts retain their existing seed and guidance fields.
- Diagnostics: baseline/target/build mismatch, stale corpus, rights/card block, late/missing IMM.021 protocol, G3 custody/artifact/configuration/request gap, held-out selection/access/candidate/reveal mismatch, paired-build input/environment/equivalence gap, stability inactive/prior-root mismatch, unratified dependency/provider, missing start event, harness fault, cancellation, resource limit, receipt truncation, and protected-data violation.
- Evidence and provenance: receipt binds unchanged pre-run manifest, fuzz baseline, applicable IMM.021/G3/probe/stability protocols, candidate/instrumented or paired build/toolchain/dependency roots, target/corpus/seed/schedule, external artifact/configuration/request/result or probe/stability roots where applicable, guidance schema and before/after queue/corpus roots, merge/minimization receipts, general lineage, limits, executor, prospective start/end events, raw finding roots, and terminal observations.
...
- Dependencies and consumers: depends on FUZ.001–FUZ.010/FUZ.013/FUZ.016, IMM.009/IMM.011/IMM.014/IMM.017/IMM.021, FDN.015/FDN.016, and ratified D7/D-005 state as applicable; FUZ.011/FUZ.015, IMM.006/IMM.020, and IMM.019 consume receipts.
...
- Inputs and identity domain: FUZ.016 baseline, all expected/observed FUZ.014 manifests/receipts and `FuzzGuidanceReceiptId` records including general mutation/fault-seeding, G3-subset, held-out probe, reproducible-build, and active-scope stability run classes; FUZ.011 panic result; FUZ.012 unresolved witnesses/tombstones; `CheckerAssessmentState` (`Assessed(IMM.020 CheckerLineageAssessmentId)` or authenticated `Unavailable(reason, affected_rows, evidence)`); every general `OracleLineageId`; IMM.021/probe protocol status; optional predeclared recovery-calibration protocol; lane policy, dispositions, and availability.
- Preconditions: baseline and candidate/instrumented or paired build roots match every included result; expected lane/run set is closed for this aggregate; duplicate, stale, cancelled, missing, unavailable, or lineage-unknown inputs remain explicit. Checker unavailability withholds only checker-specific contributions and never converts them to pass or blocks unrelated assurance aggregation by itself. G3-subset unavailable state is distinct from unrun/stale/contaminated state; held-out results satisfy selection/access/reveal law; reproducible-build and active stability rows have their typed receipts; no-promise stability scopes create no required run. A recovery-calibration lane requires FUZ.013 access/seal compliance, frozen denominators/strata, and adjudicated-intent provenance; otherwise its envelope is unavailable or indeterminate.
...
- Outputs and outcomes: immutable `AssuranceLaneAggregateId` with per-lane expected/executed/missing/cancelled/failed/indeterminate/unavailable sets; typed G3-subset protocol/right/custody/artifact/request/result or unavailable state; held-out probe result/reveal/replacement state; paired-build equivalence/discrepancy state; active-scope stability outcomes and no-promise exclusions; general suite mutation/fault-seed accounting; guidance and queue/corpus evolution; lineage correlation/unknown summaries without numeric inflation; unresolved findings; checker assessment or authenticated unavailable state with separately retained misses when assessed; coverage links; candidate root; and exact gate contribution. The existing optional recovery-calibration envelope remains unchanged and emits no global recovery-confidence score.
- Diagnostics: baseline/build/instrumentation mismatch, missing/duplicate/stale guidance or run receipt, G3 unavailable/unrun/contaminated-state collapse, G3 custody or result gap, held-out probe selection/access/result/reveal/replacement gap, paired-build discrepancy/equivalence gap, active stability scope without receipt, no-promise scope counted as pass, feedback/queue/corpus reconciliation failure, omitted/forged/unknown lineage, unresolved panic, omitted/forged general mutation seed, general detection miss, checker-specific miss, unavailable witness, invalid disposition, calibration protocol gap, denominator/stratum drift, and aggregation fault.
- Evidence and provenance: aggregate binds baseline, every run manifest/receipt, panic result, witness status, checker assessment, IMM.021 and G3/probe protocols, G3 rights/custody/artifact/request/result roots, held-out selection/access/result/reveal/replacement roots, paired-build/equivalence roots, stability activation/prior/current roots, policy, dispositions, composer, and construction receipt.
...
- Dependencies and consumers: depends on FUZ.011/FUZ.012/FUZ.014/FUZ.016 and FUZ.013 protocol status; in state transition `general-assurance-aggregate-available -> checker-assessed-aggregate-available`, additionally consumes IMM.020 only when checker-specific contribution is requested and the matching assessment root is supplied. IMM.019 and the product/close gates consume it.
...
- Inputs and identity domain: candidate scope, FUZ.001–FUZ.010/FUZ.013 contract versions, target/corpus/fixture/rights manifests, required lanes/run classes including general mutation/fault-seeding, G3-subset, held-out probe, reproducible-build, active-scope stability, and separately retained IMM.020 checker trials; IMM.021 and lane/probe protocol roots; G3 rights/custody/external-artifact/configuration/request schema or explicit unavailable state; paired-build/equivalence schema; stability activation/prior-surface roots; exact guidance instrumentation/build relation; guidance metric/schema and queue/corpus evolution/merge/minimization policy; toolchain-role decision; symbolic dependency/provider constraints; semantic/resource ceilings; witness/disposition policy; and gate mapping.
- Preconditions: D1/D2 and D7 are human-ratified; target/card/rights gaps are blocked rather than guessed; any campaign trial/G3/probe lane binds its predating IMM.021 protocol; G3 unavailable is typed and cannot be inferred into a run; only active stability scopes require a prior root/run; D-005/runtime/crypto-crate selections remain symbolic until separate ratification; baseline is rooted before FUZ.014 manifests launch.
...
- Outputs and outcomes: immutable `FuzzBaselineId` with exact candidate; target/lane matrix; expected general, G3-subset, held-out, paired-build, active-stability, and checker-specific run classes; G3 protocol/right/custody/artifact/request or unavailable state; probe protocol/selection mapping; paired-build/equivalence schema; stability activation/prior roots and no-promise exclusions; corpus/fixture roots and availability; guidance instrumentation/metric/schema; initial queue/corpus roots; evolution/merge/minimization policy; toolchain roles; symbolic provider blocks; limits; witness policy; gate links; and known gaps.
...
- Evidence and provenance: baseline binds ratified decisions, candidate/contract/target/corpus manifests, IMM.021 and lane/probe protocol roots, G3 rights/custody/artifact/configuration/request schema or unavailable record, paired-build/equivalence schema, stability activation/prior roots, rights/card roots, policy authors/review, construction tool, known gaps, and pre-run timestamp in a separate envelope.
...
- Falsifiers and Gauntlet: baseline after run, omitted hostile surface, missing CLI target, missing/conflated general or G3/probe/paired-build/active-stability class, G3 unavailable state lacking protocol/reason, probe without selection mapping, stability run for no-promise scope, duplicated corpus partition, absent/stale guidance instrumentation, feedback schema without queue/corpus receipt, hidden provider selection, target without gate, run class without receipt schema, stale candidate, and protected identity leak.
...
- Dependencies and consumers: depends on FUZ.001–FUZ.010/FUZ.013, IMM.001/IMM.009/IMM.011/IMM.021, FDN.015/FDN.016, and ratified D1/D2/D7 plus D-005 state; FUZ.011/FUZ.014/FUZ.015 and IMM.019 consume it.
...
source snapshot
  ...
  -> immutable DOC.009 strict result + RPT.003 security inventory + RPT.010 ValidationReport
  -> RPT.001 OpenReport with exact recovery status + optional replay/evidence package
```
...
20. Emit the unchanged DOC.009/RPT.003/RPT.010 strict and validation artifacts plus separately rooted RecoveryReport and RPT.007 enrichment containing source, graph, every basis/view/receipt/hypothesis chain, container/virtual provenance, credentials-used status without secret material, limits, diagnostics, and no-claims. Then mint a new recovery-aware RPT.001 OpenReport body that binds those exact roots and supersedes, without rewriting, any earlier not-requested OpenReport root.
...
Before candidate work, IMM.018 opens the prospective ledger under ratified D1/D2/D6 and IMM.017 appends each attempt/failure/intervention event without waiting for final artifacts. DOC.009/RPT.003/RPT.010 produce immutable strict and validation semantics; RPT.002/RPT.007 add separately rooted recovery semantics without consuming RPT.001; RPT.001 composes the candidate OpenReport grammar from those immutable roots and exact typed availability; RPT.004 keeps host observations separate; RPT.009 packages only the exact replay relation supported by its capture/determinism class and never uses ambient networking. RPT.006 is an unaudited candidate manifest, IMM.012 emits the later audit receipt, and RPT.008 binds that manifest to the independently produced IMM.012, FUZ.011, FUZ.015, and IMM.020 roots without rewriting any input.
...
| Bounded recovery | immutable DOC.009/RPT.003/RPT.010 artifacts + REC bases/consequences + DOC view identities + RPT.007 | `RecoveryReport`, enrichment, recovery-aware `OpenReport`, and candidate views | Unblocks task-local read selection; blocks singular source/security claim if alternatives differ. |
...
| Pre-view identity domains | Human-ratified record classifying REV.008, BYT.005, and SEC.008 under controlling Rev 7 §9.6 revision-discovery or §10.10 declared-domain law with their exact existing source/base-graph/frontier-or-view-basis/security-context inputs; if that classification is rejected, a versioned kernel successor plus affected identity/cache/report migration and claim-lapse review | Structural-signature discovery, preservation overlay, pre-view security-scope planning, and dependent final-graph/view paths remain unavailable; no owner prose or final-view back-edge supplies the missing authority. |
...
| Campaign evaluation protocol — pre-candidate | Human/steward-owned IMM.021 `EvaluationProtocolCommitmentId` with independent custody, exact protocol root, amendment law, and reveal state fixed before candidate generation | Product work may proceed under its own gate; foundry-trial/probe evidence is exploratory or unavailable and campaign close cannot pass. |
...
1. Strict hostile inspection produces immutable DOC.009/RPT.003/RPT.010 strict and validation artifacts plus an RPT.001 OpenReport with every source/base-and-final-graph/strict-view identity, exact observation coverage, raw/history/object inventory including visible/shadowed/unreachable/unknown occurrence classifications, page attribute/geometry/request and cross-revision continuity evidence, metadata Info/XMP reconciliation and bounded hostile-XML handling, three separately reported validation layers, structural signatures, encryption/security inventory, resource accounting, diagnostics, and no-claims. RPT.010 binds the role-labeled validation derivation and exact checker build/protocol/schema identities; DOC.009/RPT.003/RPT.010 retain no recovery dependency, while the initial OpenReport records exact not-requested or unavailable recovery state.
2. Bounded recovery exercises every initial C1 family, explicitly including common header/EOF-placement deviations, through basis → candidate view → REC.010 consequence finalization; preserves SYN.002 offsets/leading bytes and strict truth; records all seven materiality axes with visible-appearance/extracted-text unavailability explicit; retains materially distinct and equivalence-unresolved alternatives; carries evidence, per-hypothesis cost vectors with reconciled shared/unavailable attribution, and affected objects/pages; and emits RPT.002/RPT.007 without modifying DOC.009/RPT.003/RPT.010 artifacts, graphs, source identity, or source bytes. RPT.001 then mints a new recovery-aware OpenReport root binding exact status, every admitted revision/view/hypothesis, and analyzed-or-unavailable reachability/effect coverage while preserving the earlier root through an explicit supersession link.
...
10. RPT.006 reports only candidate declarations and every decision/card/provider/absent-category block; RPT.008 alone reports admission by binding that unchanged manifest to later IMM.012, FUZ.011, and FUZ.015 roots plus `CheckerAdmissionState`. A missing, cancelled, failed, stale, or indeterminate root required by a specific row cannot pass that row. Authenticated checker unavailability withholds only checker/package/independence rows and does not strand unrelated R0 rows.
...
12. When a checker-dependent row is requested, IMM.020 records actual checker lineage and every seeded detection/miss/indeterminate outcome against the earlier IMM.014 plan; under the proposed D4 profile, any miss is unresolved Grade A and blocks that stronger row. When D4/checker assessment is unavailable, RPT.008 carries the authenticated unavailable state, withholds every checker/package/independence row, and may still admit unrelated product rows if their own evidence passes.
...
15. Customer availability is blocked until IMM.022 supplies an authenticated human-owned `VulnerabilityResponsePolicyStatusId` in adopted state with reviewed intake, triage, embargo, advisory-format, and claim-withdrawal coverage flags. A missing, draft-only, unavailable, lapsed, withdrawn, or indeterminate status does not erase internal product evidence or campaign records, but the wedge is not offered to customers; this plan drafts no policy body and takes no external action.
...
### 11.4 Gauntlet gate — versioned shell §27 C1 projection

The governing definition of done is the exact content root and section version of `shell/MONKEYBEE_SHELL_CORPUS_v8.md` §27 through proposed Constitution §34.9. This gate binds the entire G0–G7 taxonomy, G6.1, every applicable §27.1 falsifier family, and the §27.4 consequence-contract-derived release matrix. The bullets below are a C1 scope/availability projection, not a replacement ontology; an omitted, pending, or unavailable lane remains visible under the governing version, and a taxonomy/pairing/matrix-validity change is a declared kernel touch.

The C1 projection requires these assurance lanes:
...
- G3-subset: only a FUZ.016-declared `G3SubsetLaneProtocolId` with rights/custody/external-artifact/configuration/request schema may produce FUZ.014 run receipts and a FUZ.015 lane disposition. `Unavailable` is typed separately from unrun, stale, contaminated, or failed. No lane state blocks the ordinary R0 product gate, and this plan performs no external black-box contact or observation.
...
- G5/G6 subset: hostile fuzz/bomb/malformed storms plus cancellation, concurrency, determinism, cache/tenant isolation, replay, and reproducible-witness lanes for the exact candidate and declared envelopes.
- G6.1: Miri/sanitizer evidence where applicable, unsafe-capsule falsifiers, candidate-bound offline dependency advisory/license/provenance/duplicate-version audits, reproducible-build checks, privacy/taint fixtures, and a general suite mutation/fault-seeding lane retained separately from IMM.020 checker-specific seeded trials.
- G7: the end-to-end Workflow B projection plus the §11.5 held-out extension probe and inherited budget/diagnostic/evidence/schema/operational obligations. Missing steward custody, protocol commitment, or probe availability is typed campaign-gate unavailability; it blocks campaign close but does not silently fail an otherwise supported R0 product row.
...
1. an IMM.021 evaluation protocol independently custodied before candidate generation and bound through the prospective event interval;
2. one `HeldOutProbeId` selected from the independently stewarded pre-implementation sealed pool under `HeldOutProbeProtocolId`;
3. `HeldOutProbeResultId` binding the frozen protocol, access/submission budget, exact candidate artifact, terminal result whether favorable or unfavorable, reveal demotion, and replacement transition;
4. a cycle-close drift audit over the exact probed candidate with zero unresolved Grade-A findings;
5. required card coverage with no required slot pending/blocked for the declared C1 envelope;
6. an IMM.018 bootstrap predating candidate work and verified IMM.017 prospective event interval containing attempts, failures, retries, interventions, and the applicable IMM.021 protocol/amendment root;
7. FUZ.016 baseline, all FUZ.014 receipts including typed G3/probe/paired-build/active-stability states, FUZ.011 panic result, FUZ.015 aggregate, and any checker-lineage assessment required by the declared campaign claim bound to the exact candidate;
8. an IMM.019 sealed `CycleTrialRecordId` over those prospective events, versioned aggregation law, typed probe result, and final roots under ratified D1/D2/D6; and
9. commitment of that sealed record through the human-established substrate, not a bare git timestamp, only after hash/canonicalization ratification.
...
| Checker isolation unavailable | RPT.008 authenticates the unavailable state; package/checker/independence rows are withheld while unrelated R0 rows retain their own admission path | No independent-check support; campaign claim tier degrades. |
| External G3-subset unavailable | No change to ordinary product behavior | FUZ.015 records typed unavailable state distinct from unrun/stale/contaminated; no fabricated external evidence. |
...
| Dedicated typed explanation graph and `mb explain` query surface | C5 | C1 object provenance, resolution paths/cycles, exact/virtual/enclosing spans, and visible/shadowed/unreachable/unknown occurrence state remain queryable through RPT.001 and the existing `mb inspect`/`mb revisions`/`mb objects` projections. FDN.011 additionally emits `ObjectExplanationHandoffId` over those exact nodes, edges, occurrence states, and unresolved inverse-causality gaps for the later owner. C1 adds no dedicated explanation API, pixel/glyph/text/signature-impact/transform/divergence explanation, or unique inverse attribution. |
| General public Rust-facade stability and compatibility promise | C4 release-engineering gate | C1 records `NoStabilityPromise` for the general facade. FDN.015/IMM.006/FUZ.014 check only schema, diagnostic, and CLI scopes whose own major-version stability is already active; broader activation requires a prior admitted surface root and explicit release authority. |
