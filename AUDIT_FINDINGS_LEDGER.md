...
**Purpose:** Single home for every finding, its severity, and its fix path — so Cycle 0 can apply fixes from one list and future audits can verify against drafting history instead of re-litigating. Grade A = directly confirmed in the artifact (grep/derivation); grade B = confirmed by close reading; grade C = judgment call.

**Status codes:** `OPEN-C0` (fix at Cycle 0 per Work Order §2) · `RESOLVED-CHARTER` (incorporated into Charter v1.0) · `RESOLVED-REASONING` (correction recorded in Reasoning doc) · `NOTED` (no action required) · `SOFTENED` (finding downgraded on re-examination).
...
| R1-5 | The evidence machinery assumes an unprovisioned human ecology (stewards ×23, scouts, adjudicators, two-person SpecCard review) with no recruitment/cost/degradation plan; absent from §35.1 assumptions | A (absence) | OPEN-C0 + RESOLVED-CHARTER (day-zero track, degradation law) |
...
| R1-10 | Meta: the plan hand-restates its laws 4–6× each (idempotency ~25, nonce ~23), violating its own §29.5 single-source principle; R1-1 is the empirical proof of the resulting drift | A | RESOLVED via v8 split (one normative home per law) |
...
Campaign-draft bugs, all **RESOLVED-CHARTER** in v1.0:
...
| C-9 | Charter-as-precommitment via git alone is weak under §33.4 | Commitment substrate required; Charter hash = ledger entry #1 |
...
2. Zero PDF-normative errors found across three deliberate hunts (inheritance, text state, encryption scoping, filter chaining, blend algebra, CMaps). The domain content is clean; the defect surface is coordination and evidence plumbing.
3. The two Grade-A defects in this ecology to date are both duplication drift (R1-1, R2-N7). The v8 single-normative-home split and blocking drift audits are the countermeasures; their effectiveness is measurable from Cycle 1's close gate onward.

---

*End of Ledger v1.0. Next audit round should append, never rewrite.*
